...
function EditDialog({
	...
	onClose: () => void;
	onSaved: (updated: SaijSentencia) => void;
}) {
				...
				<Stack spacing={2} mt={0.5}>
					<FormControl size="small" fullWidth>
						...
						<Select
							value={form.status}
							label="Estado"
							onChange={(e) => setForm((f) => ({ ...f, status: e.target.value as SaijSentencia["status"] }))}
						>
							...
							{fallo.fuero && (
								<Chip
									...
									sx={{
										fontSize: 11,
										height: 20,
										bgcolor: alpha(FUERO_COLOR[fallo.fuero] || "#888", 0.12),
										color: FUERO_COLOR[fallo.fuero] || "#888",
										fontWeight: 700,
									}}
								/>
							)}
							<Chip label={STATUS_LABEL[fallo.status] || fallo.status} size="small" color={STATUS_COLOR[fallo.status] || "default"} />
							{sumarioCount > 0 && <Chip label={`${sumarioCount} sumario${sumarioCount > 1 ? "s" : ""}`} size="small" variant="outlined" />}
						</Stack>
					</Box>
					<IconButton size="small" onClick={onClose}>
						<CloseCircle size={18} />
										...
										<Paper key={i} variant="outlined" sx={{ p: 1, display: "flex", alignItems: "center", gap: 1 }}>
											...
											<Chip
												...
												sx={{
													fontSize: 10,
													height: 20,
													bgcolor: alpha(FUERO_COLOR[r.fuero] || "#888", 0.1),
													color: FUERO_COLOR[r.fuero] || "#888",
												}}
											/>
						...
						<Paper variant="outlined" sx={{ p: 2 }}>
							<Stack direction="row" alignItems="center" justifyContent="space-between">
								<Box>
									...
									<Typography variant="caption" color="text.secondary">
										Modifica estado, título, tribunal, URL del PDF o mensaje de error.
									</Typography>
								</Box>
								<Button variant="outlined" startIcon={<Edit size={16} />} onClick={onEdit} sx={{ whiteSpace: "nowrap" }}>
									Editar
								</Button>
							</Stack>
						</Paper>
						<Paper variant="outlined" sx={{ p: 2, borderColor: "error.light" }}>
							<Stack direction="row" alignItems="center" justifyContent="space-between">
								<Box>
									...
									<Typography variant="caption" color="text.secondary">
										Elimina permanentemente este documento. Esta acción no se puede deshacer.
									</Typography>
								</Box>
								<Button variant="outlined" color="error" startIcon={<Trash size={16} />} onClick={onDelete} sx={{ whiteSpace: "nowrap" }}>
									Eliminar
								</Button>
							</Stack>
						</Paper>
							...
							{sumario.fuero && (
								<Chip
									...
									sx={{
										fontSize: 11,
										height: 20,
										bgcolor: alpha(FUERO_COLOR[sumario.fuero] || "#888", 0.12),
										color: FUERO_COLOR[sumario.fuero] || "#888",
										fontWeight: 700,
									}}
								/>
							)}
					...
					<Grid container spacing={1.5}>
						{[
							...
							<Grid item xs={12} sm={6} key={r.label}>
								...
								<Typography variant="body2" fontFamily={r.label.includes("ID") ? "monospace" : undefined}>
									{r.value}
								</Typography>
							</Grid>
						))}
						{sumario.expediente?.numero && (
							<Grid item xs={12} sm={6}>
								...
								<Typography variant="body2">
									{sumario.expediente.numero}/{sumario.expediente.año}
								</Typography>
							</Grid>
						)}
...
export default function JurisprudenciaSaijPage() {
	...
	return (
		<MainCard
			...
			secondary={
				<Stack direction="row" spacing={1} alignItems="center">
					...
					<CrossViewLinks current="datos" to={{ worker: "/admin/workers/saij?tab=estado", flujo: "/admin/flujos?tab=saij" }} />
		...
		>
			...
			{selected && selected.saijType === "sumario" && <SumarioDetail sumario={selected} onClose={() => setSelected(null)} />}
