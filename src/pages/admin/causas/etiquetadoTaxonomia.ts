...
export const DIM_LABELS: Record<DimKey, DimDef> = {
	...
	resultado: {
		...
		opciones: [
			...
			["desierto", "Desierto"],
			["homologa", "Homologa"],
			["declara", "Declara (de oficio)"],
			["impone", "Impone / distribuye"],
			["regula", "Regula (fija monto)"],
			["no_aplica", "No aplica"],
			["otro", "Otro"],
		],
		grupos: [
			...
			{ titulo: "De oficio (sin petición de parte: incompetencia, caducidad, rebeldía…)", valores: ["declara"] },
			{ titulo: "Accesorias de primer grado (la distribución fina va en el detalle de la fila)", valores: ["impone", "regula"] },
...
const ACTOS_PROCESALES_BASE: [string, string][] = [
	...
	["tiene_por_contestada_demanda", "Tiene por contestada la demanda (traba de litis)"],
...
> = {
	...
	// Hito de traba de litis: la contestación queda incorporada al proceso.
	tiene_por_contestada_demanda: { tipoResolucion: "providencia_simple", materia: "fondo", funcion: "impulso", resultado: "no_aplica" },
	...
	// La Sala recibe el expediente elevado y hace saber su integración: entrada
	...
	// Acepta un desistimiento voluntario. La materia sigue al objeto desistido
	...
	// Decreto de subasta en la etapa de ejecución (bienes embargados): fija modalidad, base y
	// martillero. La designación del martillero va como secundario (designa_perito).
	ordena_subasta: {
		tipoResolucion: "providencia_simple",
		materia: "ejecucion",
		contexto: "ejecucion",
...
const OBJETOS_DECIDIDOS_BASE: [string, string][] = [
	...
	["nulidad", "Nulidad"],
	["excepcion_incompetencia", "Excepción de incompetencia (de parte)"],
	["incompetencia_de_oficio", "Incompetencia de oficio (del juzgado)"],
	["rebeldia", "Rebeldía / contumacia (art. 59 CPCCN / 71 LO)"],
