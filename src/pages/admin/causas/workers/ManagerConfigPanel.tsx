...
import { Refresh2, Warning2, TickCircle, ArrowRotateRight, Cpu, Timer, People, Setting2 } from "iconsax-react";
...
const ManagerConfigPanel: React.FC = () => {
	...
	const hasScheduleChanges =
		editedSettings.workDays !== undefined || editedSettings.workStartHour !== undefined || editedSettings.workEndHour !== undefined;
									...
									<Table size="small">
										...
										<TableBody>
											...
											<TableRow sx={{ "& td": { fontWeight: "bold", borderTop: 2, borderColor: "divider" } }}>
												...
												<TableCell align="center">{Object.values(status.workers).reduce((sum, count) => sum + count, 0)}</TableCell>
												<TableCell align="center">
													{status.pending ? Object.values(status.pending).reduce((sum, count) => sum + count, 0) : 0}
												</TableCell>
											</TableRow>
										</TableBody>
									</Table>
									...
									<Grid item xs={12}>
										<Alert severity="warning" variant="outlined" icon={<Warning2 size={20} />}>
											...
											<Typography variant="body2">
												El manager recarga la configuracion cada 10 ciclos. Fuera del horario laboral, cada ciclo dura 5 minutos, por lo que
												los cambios pueden demorar hasta <strong>50 minutos</strong> en aplicarse. Para que los cambios tomen efecto de
												inmediato, reinicia el proceso del manager en PM2 despues de guardar.
											</Typography>
										</Alert>
									</Grid>
								)}
							...
							<Box>
								<Stack direction="row" justifyContent="space-between" alignItems="center" sx={{ mb: 1 }}>
									...
									<FormControl size="small" sx={{ minWidth: 100 }}>
										<Select value={hoursBack} onChange={(e) => setHoursBack(e.target.value as number)}>
