...
function TabPanel(props: TabPanelProps) {
	...
	return (
		<div role="tabpanel" hidden={value !== index} id={`stats-tabpanel-${index}`} aria-labelledby={`stats-tab-${index}`} {...other}>
				...
				<TabPanel value={activeTab} index={0}>
					<Stack spacing={3}>
								...
								<Stack direction={{ xs: "column", md: "row" }} spacing={2} alignItems={{ xs: "stretch", md: "center" }}>
									...
									{dateMode === "specific" && (
										<FormControl size="small" sx={{ minWidth: 200 }}>
											...
											<Select value={specificDate} label="Fecha" onChange={(e) => setSpecificDate(e.target.value)} disabled={loadingDates}>
												{availableDates.map((dateInfo) => (
													<MenuItem key={dateInfo.date} value={dateInfo.date}>
														<Stack direction="row" spacing={1} alignItems="center" justifyContent="space-between" sx={{ width: "100%" }}>
															<Typography variant="body2">{formatInTimezone(dateInfo.date, "DD/MM/YYYY")}</Typography>
															<Stack direction="row" spacing={0.5}>
																<Chip label={`${dateInfo.totalProcessed}`} size="small" sx={{ height: 18, fontSize: "0.7rem" }} />
															</Stack>
														</Stack>
													</MenuItem>
												))}
						...
						{alerts && alerts.count > 0 && (
							...
							>
								<Typography variant="subtitle2">Hay {alerts.count} alerta(s) activa(s) que requieren atención</Typography>
							</Alert>
						)}
											...
											{todayStats?.byFuero && todayStats.byFuero.length > 0 ? (
												todayStats.byFuero.map((fuero) => (
													<React.Fragment key={fuero.fuero}>
														<TableRow hover sx={{ cursor: "pointer" }} onClick={() => handleFueroExpand(fuero.fuero)}>
															<TableCell>
																<IconButton size="small">
																	{expandedFuero === fuero.fuero ? <ArrowUp2 size={16} /> : <ArrowDown2 size={16} />}
																</IconButton>
															</TableCell>
															<TableCell>
																<Stack>
															...
															<TableCell align="center">
																<Chip
																	label={
																		fuero.status === "completed" ? "Completado" : fuero.status === "running" ? "En ejecución" : fuero.status
																	}
																	size="small"
																	color={
															...
															<TableCell align="right">
																<Typography variant="body2">{formatNumber(fuero.stats?.processed || 0)}</Typography>
															</TableCell>
															<TableCell align="right">
															...
															<TableCell align="right">
																<Typography variant="body2" color={fuero.stats?.failed > 0 ? "error.main" : "text.secondary"}>
															...
															<TableCell align="right">
																{fuero.errorsCount > 0 ? (
																	<Chip label={fuero.errorsCount} size="small" color="error" variant="outlined" />
																) : (
																	<Typography variant="body2" color="text.secondary">
																		0
																	</Typography>
																...
																<Collapse in={expandedFuero === fuero.fuero} timeout="auto" unmountOnExit>
																	<Box sx={{ p: 3, bgcolor: alpha(theme.palette.grey[500], 0.05) }}>
																		{loadingDetails ? (
																					...
																					<Card variant="outlined">
																						<CardContent>
																							...
																							<Stack spacing={1}>
																								...
																								<Typography variant="body2">Tasa de error: {fueroDetails.errorRate}</Typography>
																								<Divider />
																								<Typography variant="caption" color="text.secondary">
																									Última actualización: {formatDate(fueroDetails.lastUpdate)}
																								</Typography>
																							</Stack>
																						</CardContent>
																					</Card>
																						...
																						<CardContent>
																							...
																							<Stack spacing={0.5}>
																								<Box sx={{ display: "flex", justifyContent: "space-between" }}>
																									...
																									<Typography variant="body2">{fueroDetails.stats?.publicCausas || 0}</Typography>
																								</Box>
																								<Box sx={{ display: "flex", justifyContent: "space-between" }}>
																									<Typography variant="body2" color="text.secondary">
																										Causas privadas:
																									</Typography>
																									<Typography variant="body2">{fueroDetails.stats?.privateCausas || 0}</Typography>
																								</Box>
																								<Box sx={{ display: "flex", justifyContent: "space-between" }}>
																									<Typography variant="body2" color="text.secondary">
																										Omitidos:
																									</Typography>
																									<Typography variant="body2">{fueroDetails.stats?.skipped || 0}</Typography>
																								</Box>
																								<Divider />
																								<Box sx={{ display: "flex", justifyContent: "space-between" }}>
																									<Typography variant="body2" color="text.secondary">
																									...
																									<Typography variant="body2">{fueroDetails.stats?.captchaAttempts || 0}</Typography>
																								</Box>
																								<Box sx={{ display: "flex", justifyContent: "space-between" }}>
																									<Typography variant="body2" color="text.secondary">
																										Captchas exitosos:
																									...
																									{fueroDetails.alerts.map((alert: any, idx: number) => (
																										<Alert
																											...
																											action={
																												<Button
																													size="small"
																													onClick={() => handleAcknowledgeAlert(fuero.fuero, alert.type)}
																												>
																													OK
																												</Button>
																											}
																									...
																									<Table size="small">
																										...
																										<TableBody>
																											{fueroErrors.slice(0, 10).map((error, idx) => (
																												<TableRow key={idx}>
																													<TableCell>
																														<Typography variant="caption">{formatDate(error.timestamp)}</Typography>
																													</TableCell>
																													<TableCell>
																														<Chip label={error.errorType} size="small" color="error" variant="outlined" />
																													</TableCell>
																													<TableCell>
																														{error.number && error.year ? `${error.number}/${error.year}` : "-"}
																													</TableCell>
																													<TableCell>
																														<Typography
																															variant="caption"
																			...
																			<Typography color="text.secondary">No hay datos disponibles para este fuero</Typography>
																		)}
																	</Box>
																</Collapse>
															</TableCell>
