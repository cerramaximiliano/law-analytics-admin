...
const MisCausasHelpTab: React.FC = () => {
			...
			<Card variant="outlined" sx={{ bgcolor: "background.default" }}>
				<CardContent>
					...
					<Typography variant="body2" paragraph>
						El <strong>Scraping Worker Manager</strong> es un proceso orquestador que controla el ciclo de vida de todos los workers de
						scraping del sistema "Mis Causas" del Poder Judicial de la Nación (PJN). Opera como un proceso PM2 independiente que monitoriza
						la cola de trabajo pendiente en MongoDB y usa la API programática de PM2 para iniciar, detener y escalar workers
						automáticamente.
					</Typography>
					<Typography variant="body2" paragraph>
						Toda la configuración se gestiona desde MongoDB (colección <code>scraping-manager-state</code>, documento <code>config</code>) y
						se recarga periódicamente: los cambios se aplican sin necesidad de reiniciar el manager.
					</Typography>
			...
			<Card variant="outlined" sx={{ bgcolor: "background.default" }}>
				<CardContent>
					...
					<Section title="Función">
						<Typography variant="body2">
							Procesa credenciales PJN que aún no han sido verificadas. Realiza el login SSO, confirma la identidad y cantidad de causas del
							usuario, extrae todas las causas y crea las carpetas correspondientes.
						</Typography>
					</Section>
				...
				<CardContent>
					<Stack direction="row" spacing={1} alignItems="center" sx={{ mb: 2 }}>
						...
						<Typography
							...
							sx={{ fontFamily: '"Geist Variable", "Geist", system-ui, sans-serif', letterSpacing: "-0.02em", fontWeight: 600 }}
						>
							Sync Completa (on-demand)
						</Typography>
					</Stack>

					<Section title="Función">
						<Typography variant="body2">
							Realiza una sincronización completa de todas las causas de un usuario específico. Generalmente se ejecuta a pedido (triggered
							por la API cuando un usuario solicita re-sincronización).
						</Typography>
					</Section>
  ...
  enabled: true`}</CodeBlock>
					...
					<Section title="Snapshots">
						<Typography variant="body2">
							Genera checksums MD5 de cada página de resultados, guardando <code>firstCaratula</code> y <code>lastCaratula</code> por
							página. Estos snapshots permiten al worker de actualización incremental detectar cambios sin re-extraer todo. Requiere
							ordenamiento por CARATULA para funcionar correctamente.
						</Typography>
					</Section>
				</CardContent>
			</Card>
			...
			<Card variant="outlined" sx={{ bgcolor: "background.default" }}>
				<CardContent>
					...
					<Section title="Función">
						<Typography variant="body2">
							Detecta causas nuevas de forma eficiente sin re-extraer todas las causas. Compara checksums de páginas con los snapshots
							guardados previamente para identificar solo los cambios.
						</Typography>
					</Section>
  ...
  syncMetadata.sortMethod: 'CARATULA' o 'CARÁTULA'`}</CodeBlock>
			...
			<Card variant="outlined" sx={{ bgcolor: "background.default" }}>
				<CardContent>
					...
					<Section title="Función">
						<Typography variant="body2">
							Actualiza los movimientos de causas marcadas como <code>isPrivate: true</code>. Estas causas no son accesibles sin
							autenticación, por lo que requiere login SSO para consultar cada una individualmente.
						</Typography>
					</Section>
				...
				<CardContent>
					<Stack direction="row" spacing={1} alignItems="center" sx={{ mb: 2 }}>
						...
						<Typography
							...
							sx={{ fontFamily: '"Geist Variable", "Geist", system-ui, sans-serif', letterSpacing: "-0.02em", fontWeight: 600 }}
						>
							Actualización de Movimientos (Causas Update)
						</Typography>
					</Stack>

					<Section title="Función">
						<Typography variant="body2">
							Actualiza los movimientos de TODAS las causas vinculadas a credenciales de usuario mediante login SSO al portal PJN. Usa un
							algoritmo de comparación por cantidad para detectar movimientos nuevos de forma eficiente. Opera en 3 fases con prioridad
							decreciente.
						</Typography>
					</Section>
					...
					<Section title="Fases de Ejecución">
						...
						<Typography variant="body2" sx={{ mb: 0.5 }}>
							Cuando el <code>credentials-processor</code> termina de verificar una credencial y crear causas/folders, las causas quedan sin
							movimientos. Esta fase los descarga inmediatamente.
						</Typography>
						<BulletList
							items={[
					...
					<Section title="Sync Inicial — Detalle">
						<Alert severity="info" variant="outlined" sx={{ mt: 1 }}>
							<Typography variant="body2">
								...
								<code>completed</code> (listo). El manager bypasea el schedule cuando hay syncs iniciales pendientes, permitiendo que el
								worker arranque fuera de horario.
							</Typography>
						</Alert>
							...
							<table style={{ width: "100%", borderCollapse: "collapse", fontSize: "0.8rem" }}>
								<thead>
									<tr style={{ backgroundColor: alpha(theme.palette.primary.main, 0.08) }}>
										...
										<th style={{ padding: "6px 12px", textAlign: "left", border: `1px solid ${theme.palette.divider}` }}>
											Colección MongoDB
										</th>
								...
								<tbody>
									{[
										[
											"Causa",
											"causas-civil, causas-segsocial, etc.",
											"Documento del expediente judicial. Puede existir independientemente de cualquier usuario.",
										],
										["Carpeta", "folders", "Vínculo entre un usuario y una causa. Siempre pertenece a un único userId."],
										[
											"source en causa",
											"—",
											"'pjn-login' = creado por el sync. 'scraping' = encontrado por búsqueda. 'cache' = desde caché PJN.",
										],
										["source en carpeta", "—", "'pjn-login' = creada por el sync. Otros valores = creada manualmente por el usuario."],
										[
											"linkedCredentials",
											"campo en causa",
											"Array de credenciales que vincularon esa causa. Permite saber qué usuarios/syncs la crearon.",
										],
  ...
  Búsqueda 2: busca carpeta por número de expediente
    ...
    → No encontrada: crea nueva carpeta source='pjn-login' (isNew=true)`}</CodeBlock>
					...
					<Alert severity="info" variant="outlined" sx={{ mt: 2, mb: 1 }}>
						<Typography variant="body2">
							<strong>Regla de propiedad:</strong> una causa con <code>source: 'pjn-login'</code> fue <strong>creada</strong> por el sync.
							Una causa con otro <code>source</code> fue <strong>vinculada</strong> por el sync (ya existía; el sync solo la encontró y la
							referencia). Esta distinción es crítica en el proceso de limpieza.
						</Typography>
					</Alert>
							...
							<table style={{ width: "100%", borderCollapse: "collapse", fontSize: "0.8rem" }}>
								...
								<tbody>
									{[
										...
										<tr key={i} style={{ backgroundColor: i % 2 === 0 ? "transparent" : alpha(theme.palette.primary.main, 0.03) }}>
											<td style={{ padding: "6px 12px", border: `1px solid ${theme.palette.divider}`, fontFamily: "monospace" }}>
												{source}
											</td>
											<td style={{ padding: "6px 12px", border: `1px solid ${theme.palette.divider}` }}>{otros}</td>
											<td style={{ padding: "6px 12px", border: `1px solid ${theme.palette.divider}`, fontWeight: i === 0 ? 600 : 400 }}>
												{accion}
											</td>
										</tr>
									))}
							...
							<table style={{ width: "100%", borderCollapse: "collapse", fontSize: "0.8rem" }}>
								...
								<tbody>
									{[
										...
										[
											"Causas cubiertas",
											"Las que tienen carpeta activa",
											"Las que quedaron sin carpeta (syncs parciales, cleanups previos)",
										],
										[
											"Por qué existe",
											"Ruta principal de análisis",
											"Una causa puede quedar 'huérfana' si su carpeta fue eliminada sin limpiar linkedCredentials. Sin el Bloque B quedaría en DB para siempre.",
										],
									].map(([label, a, b], i) => (
										<tr key={i} style={{ backgroundColor: i % 2 === 0 ? "transparent" : alpha(theme.palette.primary.main, 0.03) }}>
			...
			<Card variant="outlined" sx={{ bgcolor: "background.default" }}>
				<CardContent>
					...
					<Alert severity="warning" variant="outlined" sx={{ mb: 2 }}>
						<Typography variant="body2">
							La credencial <strong>no se elimina</strong> — se desactiva (<code>enabled: false</code>). Los workers filtran por{" "}
							<code>enabled: true</code> al buscar credenciales, por lo que dejan de procesar esas causas en el próximo ciclo{" "}
							<strong>sin necesidad de cambios en pjn-mis-causas</strong>.
						</Typography>
					</Alert>
...
4. Frontend: setHasCredentials(false) + dispatch(getFoldersByUserId(userId, true))`}</CodeBlock>
					...
					<Alert severity="info" variant="outlined" sx={{ mt: 2 }}>
						<Typography variant="body2">
							Para hacer un reset completo desde el admin (equivalente al script <code>cleanup-pjn-data.js</code>), usar la sección de
							Credenciales PJN → detalle de credencial → botón Reset Sync. Esa operación resetea la credencial pero la mantiene activa (
							<code>enabled: true</code>), a diferencia de la desvinculación que la desactiva.
						</Typography>
					</Alert>
				</CardContent>
			</Card>
			...
			<Card variant="outlined" sx={{ bgcolor: "background.default" }}>
				<CardContent>
					...
					<Alert severity="info" variant="outlined" sx={{ mb: 2 }}>
						<Typography variant="body2">
							<strong>Problema:</strong> cuando un usuario elimina una carpeta PJN desde la UI, el worker la recreaba en el próximo sync
							porque <code>ensureFolder()</code> no tenía forma de distinguir "carpeta eliminada intencionalmente" de "carpeta que aún no
							existe". El campo <code>excludedCausas</code> en <code>PjnCredentials</code> resuelve esto.
						</Typography>
					</Alert>
...
4. Folder.findByIdAndDelete(id) + actualización de userstats`}</CodeBlock>
					...
					<Section title="Cómo el worker respeta la exclusión">
						<Typography variant="body2" sx={{ mb: 1 }}>
							Al inicio de cada batch, <code>processCausasBatch</code> construye un <code>Set</code> con los IDs excluidos. Si{" "}
							<code>existingCausa._id</code> está en el Set, la causa se salta sin crear carpeta ni vincular <code>linkedCredentials</code>.
						</Typography>
  ...
  continue;  // sin carpeta, sin linkedCredentials
							...
							<table style={{ width: "100%", borderCollapse: "collapse", fontSize: "0.8rem" }}>
								...
								<tbody>
									{[
										...
										[
											"Usuario re-vincula la misma cuenta PJN",
											"excludedCausas persiste → las causas eliminadas siguen excluidas (comportamiento esperado)",
										],
										[
											"Usuario quiere volver a ver la causa",
											"Eliminar la entrada en excludedCausas directamente en MongoDB (no hay UI aún)",
										],
										[
											"Eliminación en bulk (hasta 50 carpetas)",
											"Carga credenciales PJN antes del loop, aplica exclusiones en batch al final",
										],
									].map(([esc, comp], i) => (
			...
			<Card variant="outlined" sx={{ bgcolor: "background.default" }}>
				<CardContent>
					...
					<Alert severity="info" variant="outlined" sx={{ mb: 2 }}>
						<Typography variant="body2">
							Cuando una causa desaparece del portal "Mis Causas" del PJN (archivada, desvinculada por el tribunal, etc.), el usuario tiene
							un folder activo sin correspondencia en el portal. El campo <code>pjnNotFound</code> en <code>Folder</code> permite detectarlo
							y mostrarlo en la UI con un indicador ámbar.
						</Typography>
					</Alert>
					...
					<Section title="Cuándo se activa la detección">
						<Typography variant="body2" sx={{ mb: 1 }}>
							...
							<strong>el total de causas en el portal disminuye</strong> (<code>currentTotal &lt; previousTotal</code>). En ese caso, el
							loop de escaneo elimina su condición de early-stop y recorre <strong>todas las páginas</strong>, acumulando las claves de
							todas las causas visibles. El sync inicial (<code>processFromSyncQueue</code>) también llama automáticamente a{" "}
...
4. Aplica todas las actualizaciones en un único bulkWrite`}</CodeBlock>
						<Alert severity="warning" variant="outlined" sx={{ mt: 1 }}>
							<Typography variant="body2">
								<code>scrapedCausaKeys</code> debe ser la lista <strong>completa</strong> del portal. Llamar con listas parciales marcaría
								incorrectamente como "no encontradas" las causas de páginas no escaneadas.
							</Typography>
						</Alert>
					</Section>
								...
								<tbody>
									{[
										...
										<tr key={i} style={{ backgroundColor: i % 2 === 0 ? "transparent" : alpha(theme.palette.warning.main, 0.03) }}>
											...
											<td style={{ padding: "6px 12px", border: `1px solid ${theme.palette.divider}`, fontWeight: i < 2 ? 600 : 400 }}>
												{result}
											</td>
										</tr>
									))}
							...
							<table style={{ width: "100%", borderCollapse: "collapse", fontSize: "0.8rem" }}>
								...
								<tbody>
									{[
										[
											"excludedCausas",
											"Usuario elimina carpeta PJN",
											"Worker omite la causa en el próximo sync",
											"Admin/MongoDB: $pull excludedCausas",
										],
										[
											"pjnNotFound",
											"Causa desaparece del portal",
											"Worker marca el folder (no lo borra)",
											"Automática en el próximo escaneo completo",
										],
			...
			<Card variant="outlined" sx={{ bgcolor: "background.default" }}>
				<CardContent>
					...
					<Section title="Auto-Escalado">
						<Typography variant="body2" paragraph>
							El manager consulta la profundidad de la cola (documentos pendientes en MongoDB) y decide cuántas instancias necesita cada
							worker:
						</Typography>
						<BulletList
							items={[
