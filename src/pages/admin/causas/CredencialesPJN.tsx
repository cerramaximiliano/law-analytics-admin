...
const CredencialesPJN = () => {
	...
	}, [hasInProgress, page, rowsPerPage, sortBy, sortOrder, syncStatusFilter, verifiedFilter, enabledFilter, searchText]);
	...
	const handleToggleEnabled = async (credential: PjnCredential) => {
		try {
			const response = await pjnCredentialsService.toggleCredential(credential._id, !credential.enabled);
			if (response.success) {
	...
	const handleDelete = async () => {
		...
		try {
			const response = await pjnCredentialsService.deleteCredential(deleteDialog.credential._id);
			if (response.success) {
				enqueueSnackbar(response.message, { variant: "success" });
	...
	return (
		<MainCard title="Credenciales PJN">
									...
									<TableRow>
										...
										<TableCell align="center">
											<Box sx={{ display: "flex", alignItems: "center", justifyContent: "center", gap: 0.5 }}>
												Estado
												<Tooltip
													title="Estado del ciclo de sincronización: pending (en espera), in_progress (procesando), completed (exitoso), error (con fallas), never_synced (nunca ejecutado)."
													arrow
													placement="top"
												>
													<Box component="span" sx={{ display: "inline-flex", cursor: "help", opacity: 0.5 }}>
										...
										<TableCell align="center">
											<Box sx={{ display: "flex", alignItems: "center", justifyContent: "center", gap: 0.5 }}>
												Verificado
												<Tooltip
													title="El login al portal PJN fue exitoso y se confirmó el número total de causas disponibles para este CUIL."
													arrow
													placement="top"
												>
													<Box component="span" sx={{ display: "inline-flex", cursor: "help", opacity: 0.5 }}>
										...
										<TableCell align="right">
											<Box sx={{ display: "flex", alignItems: "center", justifyContent: "flex-end", gap: 0.5 }}>
												Causas
												<Tooltip
													title="Causas procesadas / Causas esperadas según el portal PJN en el último ciclo de sincronización."
													arrow
													placement="top"
												>
													<Box component="span" sx={{ display: "inline-flex", cursor: "help", opacity: 0.5 }}>
										...
										<TableCell align="right">
											<Box sx={{ display: "flex", alignItems: "center", justifyContent: "flex-end", gap: 0.5 }}>
												Carpetas
												<Tooltip
													title="Carpetas nuevas creadas en el último ciclo / Total de carpetas con fuente PJN actualmente en la base de datos."
													arrow
													placement="top"
												>
													<Box component="span" sx={{ display: "inline-flex", cursor: "help", opacity: 0.5 }}>
										...
										<TableCell align="right">
											<Box sx={{ display: "flex", alignItems: "center", justifyContent: "flex-end", gap: 0.5 }}>
												Movimientos
												<Tooltip
													title="Total de movimientos judiciales en carpetas con fuente PJN (suma de movementsCount)."
													arrow
													placement="top"
												>
													<Box component="span" sx={{ display: "inline-flex", cursor: "help", opacity: 0.5 }}>
										...
										<TableCell>
											<Box sx={{ display: "flex", alignItems: "center", gap: 0.5 }}>
												Último Mov.
												<Tooltip
													title="Fecha del movimiento judicial más reciente registrado en carpetas PJN del usuario."
													arrow
													placement="top"
												>
													<Box component="span" sx={{ display: "inline-flex", cursor: "help", opacity: 0.5 }}>
										...
										<TableCell align="right">
											<Box sx={{ display: "flex", alignItems: "center", justifyContent: "flex-end", gap: 0.5 }}>
												Syncs
												<Tooltip
													title="Cantidad de sincronizaciones exitosas completadas históricamente para esta credencial."
													arrow
													placement="top"
												>
													<Box component="span" sx={{ display: "inline-flex", cursor: "help", opacity: 0.5 }}>
										...
										<TableCell align="center">
											<Box sx={{ display: "flex", alignItems: "center", justifyContent: "center", gap: 0.5 }}>
												Errores
												<Tooltip
													title="Errores consecutivos acumulados desde la última sincronización exitosa. Se resetea al completar un ciclo sin errores."
													arrow
													placement="top"
												>
													<Box component="span" sx={{ display: "inline-flex", cursor: "help", opacity: 0.5 }}>
										...
										<TableCell align="center">
											<Box sx={{ display: "flex", alignItems: "center", justifyContent: "center", gap: 0.5 }}>
												Acciones
												<Tooltip
													title="Ver JSON completo · Activar/Desactivar · Resetear verificación (mantiene carpetas) · Resetear sync completo (elimina carpetas y causas) · Eliminar credencial."
													arrow
													placement="top"
												>
													<Box component="span" sx={{ display: "inline-flex", cursor: "help", opacity: 0.5 }}>
													...
													<Tooltip
														title={
															cred.byFuero && Object.keys(cred.byFuero).length > 0
																? Object.entries(cred.byFuero)
																		.map(([fuero, count]) => `${fuero}: ${count}`)
																		.join(", ")
																: "Sin datos de fuero"
														}
													>
												...
												<TableCell align="center">
													<Stack direction="row" spacing={0.5} justifyContent="center">
														<Tooltip title="Ver JSON">
															<IconButton size="small" onClick={() => handleViewRaw(cred)} color="default">
																<Eye size={18} />
															</IconButton>
														</Tooltip>
														...
														<Tooltip title="Resetear sincronización">
															<IconButton size="small" onClick={() => handleOpenResetSync(cred)} color="warning">
																<Broom size={18} />
															</IconButton>
														</Tooltip>
														<Tooltip title="Eliminar">
															<IconButton size="small" onClick={() => setDeleteDialog({ open: true, credential: cred })} color="error">
																<Trash size={18} />
															</IconButton>
														</Tooltip>
													</Stack>
				...
				<Grid container spacing={2}>
					...
					{stats?.syncActivity && (
						<>
							...
							{[
								...
								{
									value: `${stats.syncActivity.successRate}%`,
									label: "Tasa de Éxito (7d)",
									color: stats.syncActivity.successRate >= 90 ? "success.main" : "warning.main",
								},
								{
									value: stats.syncActivity.avgDurationMs > 0 ? `${Math.round(stats.syncActivity.avgDurationMs / 1000)}s` : "-",
												...
												(() => {
													...
													for (let i = 0; i < grouped.length; i++) {
														...
														if (isMulti && isFirstInCycle) {
															...
															rows.push(
																<TableRow
																	key={`cycle-${run.cycleId}`}
																	sx={{ backgroundColor: theme.palette.mode === "dark" ? `${cycleColor}22` : `${cycleColor}0C` }}
																>
																	<TableCell colSpan={10} sx={{ borderLeft: `4px solid ${cycleColor}`, py: 0.5 }}>
																		<Stack direction="row" spacing={1} alignItems="center" flexWrap="wrap">
																			...
																			<Typography variant="caption" color="text.secondary">
																				Totales: {totals.processed}/{totals.causas} causas · {totals.updated} actualizadas ·{" "}
																				{totals.newMovs} mov. nuevos · {totals.errors} errores · {formatDurationSecs(totals.duration)}
																			</Typography>
																		</Stack>
																	</TableCell>
																</TableRow>,
																...
																sx={
																	isMulti
																		? {
																				...
																				"&:hover": {
																					backgroundColor: theme.palette.mode === "dark" ? `${cycleColor}28` : `${cycleColor}12`,
																				},
																		  }
																		: undefined
																}
																	...
																	<Stack>
																		<Stack direction="row" spacing={0.5} alignItems="center">
																			{isMulti && !isFirstInCycle && (
																				<Typography variant="caption" sx={{ color: cycleColor, fontWeight: 700, mr: 0.5 }}>
																					↳
																				</Typography>
																			)}
																			<Typography variant="body2" fontWeight={500}>
												...
												<TableRow key={idx} hover>
													...
													<TableCell>
														<Tooltip title={err.error?.message || "Sin mensaje"}>
															<Typography
																variant="caption"
																sx={{ maxWidth: 250, display: "block", overflow: "hidden", textOverflow: "ellipsis", whiteSpace: "nowrap" }}
															>
					...
					{portalStatus && (
						<>
							...
							<Grid item xs={12}>
								<Alert severity={portalStatus.activeIncident ? "error" : "success"} variant="outlined">
									{portalStatus.activeIncident ? (
										<Typography variant="body2">
											Incidente activo:{" "}
											<strong>
											...
											{" | Errores: "}
											<strong>{portalStatus.activeIncident.totalErrors}</strong>
											{" | Credenciales afectadas: "}
											<strong>{portalStatus.activeIncident.affectedCredentialsCount}</strong>
										</Typography>
									) : (
										<Typography variant="body2">
											Servicio operativo
											...
											{portalStatus.credentialsWithPortalErrors > 0 && (
												<>
													{" "}
													| <strong>{portalStatus.credentialsWithPortalErrors}</strong> credencial(es) con errores de portal en ultimos 30
													min
												</>
											)}
										</Typography>
											...
											{portalStatus.recentIncidents.map((incident: any) => (
												<TableRow key={incident._id} hover>
													<TableCell>
														<Chip
															label={
																incident.type === "portal_down" ? "Caida" : incident.type === "portal_degraded" ? "Degradado" : "Login"
															}
															size="small"
			...
			>
				...
				<DialogContent>
					<DialogContentText>
						¿Estás seguro de resetear la sincronización de <strong>{resetSyncDialog.credential?.userName}</strong> (
						{resetSyncDialog.credential?.userEmail})?
					</DialogContentText>
					<Box component="ul" sx={{ mt: 1, pl: 2 }}>
						<li>
							<Typography variant="body2">Eliminará los folders creados por la sincronización PJN</Typography>
						</li>
						<li>
							<Typography variant="body2">Eliminará las causas creadas exclusivamente por esta sincronización</Typography>
						</li>
						<li>
							<Typography variant="body2">Desvinculará al usuario de causas compartidas</Typography>
						</li>
						<li>
							<Typography variant="body2">Reseteará el estado de sincronización</Typography>
						</li>
					</Box>
			...
			>
				...
				<DialogContent>
					<DialogContentText sx={{ mb: 2 }}>Ingresá los datos de la credencial PJN para vincular a un usuario.</DialogContentText>
			...
			<Dialog open={detailDialog.open} onClose={closeDetailDialog} maxWidth="lg" fullWidth PaperProps={{ sx: { height: "85vh" } }}>
				<DialogTitle>
					<Stack direction="row" justifyContent="space-between" alignItems="center">
								...
								<>
									...
									<Box sx={{ mb: 2 }}>
										{[
											...
											{
												label: "Verificacion",
												value: formatDate(detailDialog.jsonData.verifiedAt),
												delay: calcDelay(detailDialog.jsonData.createdAt, detailDialog.jsonData.verifiedAt),
											},
											{
												label: "Sync de carpetas",
												value: detailDialog.jsonData.firstSync?.date ? formatDate(detailDialog.jsonData.firstSync.date) : "-",
												delay: detailDialog.jsonData.firstSync?.date
													? calcDelay(detailDialog.jsonData.createdAt, detailDialog.jsonData.firstSync.date)
													: null,
											},
											{
												label: "Sync de movimientos",
												value: formatDate(detailDialog.jsonData.initialMovementsSyncAt),
												delay: calcDelay(detailDialog.jsonData.createdAt, detailDialog.jsonData.initialMovementsSyncAt),
											},
