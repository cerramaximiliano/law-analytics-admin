...
const CarpetasNoVerificadasPjCatamarca = () => {
	...
	const fetchCausas = async (
		...
		try {
			...
			const response = await CausasPjCatamarcaService.getNonVerifiedCausas(params);
			...
			enqueueSnackbar("Error al cargar las carpetas no verificadas PJ Catamarca", {
				variant: "error",
	...
	return (
		<MainCard title="Carpetas no verificadas PJ Catamarca (App)">
			{/* Header: Resultados */}
			<Box sx={{ mb: 2 }}>
												...
												<TableCell align="center">
													<Tooltip title="Reintentar verificación (errorCount=0, la carpeta vuelve a 'pendiente')">
														<IconButton
															...
															onClick={async () => {
																try {
																	await CausasPjCatamarcaService.resetVerification(getId(causa._id));
...
export default CarpetasNoVerificadasPjCatamarca;
