...
interface CausaDetalleModalPjCatamarcaProps {
	open: boolean;
	onClose: () => void;
...
const CausaDetalleModalPjCatamarca = ({ open, onClose, causa, onCausaUpdated }: CausaDetalleModalPjCatamarcaProps) => {
	...
	const [editedCausa, setEditedCausa] = useState<Partial<CausaPjCatamarca>>({});
	const [isSaving, setIsSaving] = useState(false);

	// Estados para la lista de movimientos
	const [currentMovimientos, setCurrentMovimientos] = useState<MovimientoPjCatamarca[]>([]);
	...
	useEffect(() => {
		if (open && causa) {
			...
			setIsEditing(false);
			setEditedCausa({});
			// El campo real en pjcatamarca es `movimiento` (singular). `movimientos` es alias compat.
	...
	const handleSaveEdit = async () => {
		try {
			...
			enqueueSnackbar("Error al actualizar la causa PJ Catamarca", {
				variant: "error",
				anchorOrigin: { vertical: "bottom", horizontal: "right" },
			});
			console.error(error);
					...
					<Stack direction="row" justifyContent="space-between" alignItems="center" spacing={2}>
						<Box sx={{ flex: 1 }}>
							<Stack direction="row" spacing={1} alignItems="center" sx={{ mb: 0.5 }}>
								...
								<Chip label="PJ Catamarca" color="primary" size="small" />
				...
				<DialogContent dividers sx={{ height: "500px", overflowY: "auto" }}>
					...
					{activeTab === 0 && (
						<Grid container spacing={2}>
							...
							{/* Campos específicos PJ Catamarca */}
							{causa.juez && (
								<Grid item xs={12} md={6}>
									<Typography variant="caption" color="textSecondary">
										Juez en trámite
							...
							{/* Sala y TribunalSuperior — no aplican a PJ Catamarca (campos legacy del modal eje) */}
								...
								<>
									<TableContainer>
										<Table size="small">
											...
											<TableBody>
												{paginatedMovimientos.map((mov: MovimientoPjCatamarca, index: number) => (
...
export default CausaDetalleModalPjCatamarca;
