...
import { CausasPjCatamarcaService, CausaPjCatamarca, CausasPjCatamarcaResponse } from "api/causasPjCatamarca";
...
const LinkedCausasRow = ({
	...
	pivot: CausaPjCatamarca;
	expanded: boolean;
	onSelectCausa: (pivotId: string, causaId: string, causa: CausaPjCatamarca) => void;
}) => {
	const [linkedCausas, setLinkedCausas] = useState<CausaPjCatamarca[]>([]);
	...
	const loadLinkedCausas = async () => {
		try {
			setLoading(true);
			const response = await CausasPjCatamarcaService.getPivotLinkedCausas(getId(pivot._id));
			if (response.success) {
...
const CarpetasPivotsPjCatamarca = () => {
	const theme = useTheme();
	const { enqueueSnackbar } = useSnackbar();

	// Estados
	const [pivots, setPivots] = useState<CausaPjCatamarca[]>([]);
	const [loading, setLoading] = useState(false);
	...
	const fetchPivots = async (
		...
		try {
			...
			const response = await CausasPjCatamarcaService.getPivotCausas(params);
			...
			enqueueSnackbar("Error al cargar los pivots PJ Catamarca", {
				variant: "error",
	...
	const handleSelectCausa = (pivotId: string, causaId: string, causa: CausaPjCatamarca) => {
		setConfirmDialog({
			open: true,
			pivotId,
			causaId,
	...
	const handleConfirmSelection = async () => {
		try {
			const response = await CausasPjCatamarcaService.resolvePivot(confirmDialog.pivotId, confirmDialog.causaId);
			if (response.success) {
	...
	return (
		<MainCard title="Pivots PJ Catamarca — múltiples resultados">
			{/* Header: Resultados */}
			<Box sx={{ mb: 2 }}>
...
export default CarpetasPivotsPjCatamarca;
