...
const PjCatamarcaWorkersConfig: React.FC = () => {
	...
	const handleAcknowledgeAllAlerts = async () => {
		setActionLoading(true);
		try {
			const r = await configPjCatamarca.acknowledgeAllAlerts();
			setAlerts([]);
	...
	const handleAcknowledgeAlert = async (alertId: string) => {
		try {
			await configPjCatamarca.acknowledgeAlert(alertId);
			...
			<Grid container spacing={3}>
				...
				<Grid item xs={12}>
					<MainCard
						...
						secondary={
							<Stack direction="row" spacing={1} alignItems="center">
								<CrossViewLinks current="worker" to={{ datos: "/admin/pjcatamarca/verified-app" }} />
								<Box
									sx={{
										display: "inline-flex",
										alignItems: "center",
