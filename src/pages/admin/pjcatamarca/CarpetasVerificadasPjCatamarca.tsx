...
const getTodayUpdateInfo = (causa: CausaPjCatamarca): { isToday: boolean; count: number; hours: number[] } => {
...
const CarpetasVerificadasPjCatamarca = () => {
	const theme = useTheme();
	const { enqueueSnackbar } = useSnackbar();

	// Estados
	const [causas, setCausas] = useState<CausaPjCatamarca[]>([]);
	...
	// Modal de detalles
	const [selectedCausa, setSelectedCausa] = useState<CausaPjCatamarca | null>(null);
	...
	const fetchCausas = async (
		...
		try {
			...
			const response = await CausasPjCatamarcaService.getVerifiedCausas(params);
			...
			enqueueSnackbar("Error al cargar las carpetas verificadas PJ Catamarca", {
				variant: "error",
	...
	const handleVerDetalles = async (causa: CausaPjCatamarca) => {
		try {
			setLoadingDetail(true);
			const causaId = getId(causa._id);

			const response = await CausasPjCatamarcaService.getCausaById(causaId);
	...
	return (
		<MainCard title="Carpetas verificadas PJ Catamarca (App)">
			...
			<Grid container spacing={3}>
				...
				<Grid item xs={12}>
					{loading ? (
						...
						<Alert severity="info">No se encontraron carpetas verificadas PJ Catamarca con los filtros seleccionados</Alert>
					) : (
						<Card>
							<TableContainer>
								<Table>
...
export default CarpetasVerificadasPjCatamarca;
