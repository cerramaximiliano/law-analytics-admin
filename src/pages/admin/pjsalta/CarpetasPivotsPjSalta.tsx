...
const LinkedCausasRow = ({
	...
	pivot: CausaPjSalta;
	expanded: boolean;
	onSelectCausa: (pivotId: string, causaId: string, causa: CausaPjSalta) => void;
}) => {
	const [linkedCausas, setLinkedCausas] = useState<CausaPjSalta[]>([]);
	...
	const loadLinkedCausas = async () => {
		try {
			setLoading(true);
			const response = await CausasPjSaltaService.getPivotLinkedCausas(getId(pivot._id));
			if (response.success) {
...
const CarpetasPivotsPjSalta = () => {
	const theme = useTheme();
	const { enqueueSnackbar } = useSnackbar();

	// Estados
	const [pivots, setPivots] = useState<CausaPjSalta[]>([]);
	const [loading, setLoading] = useState(false);
	...
	const fetchPivots = async (
		...
		try {
			...
			const response = await CausasPjSaltaService.getPivotCausas(params);
			...
			enqueueSnackbar("Error al cargar los pivots PJ Salta", {
				variant: "error",
	...
	const handleSelectCausa = (pivotId: string, causaId: string, causa: CausaPjSalta) => {
		setConfirmDialog({
			open: true,
			pivotId,
			causaId,
				...
				<Grid container spacing={2} alignItems="center">
					...
					<Grid item xs={12} md={8}>
						<Alert severity="info" icon={<DocumentText size={20} />}>
							Los pivots son búsquedas que encontraron múltiples resultados en PJ Salta. Expande cada fila para ver las opciones y seleccionar la
							causa correcta.
						</Alert>
					</Grid>
				</Grid>
...
export default CarpetasPivotsPjSalta;
