/**
 * PJ Salta Workers Configuration Page
 * Comprehensive dashboard for managing PJ Salta workers with separate tabs for each worker type
 */
...
const WorkerCard: React.FC<WorkerCardProps> = ({ workerType, config, status, effectiveSchedule, onToggle, onEdit, loading }) => {
	...
	const workerLabels: Record<string, { name: string; description: string; icon: React.ReactNode }> = {
		verifier: {
			name: "Verificación",
			description: "Verifica que los expedientes existen en el sistema PJ Salta",
...
const PjSaltaWorkersConfig: React.FC = () => {
	const theme = useTheme();
	...
	const fetchData = useCallback(async () => {
		try {
			const [workersResponse, statsData, alertsData, pipelineData] = await Promise.all([
				configPjSalta.getAllWorkersConfig(),
				configPjSalta.getTodaySummary(),
				configPjSalta.getAlerts(false),
				// Tolerante: el resto del dashboard no depende de esta llamada.
				CausasPjSaltaService.getPipelineStats().catch(() => null),
			]);
			...
			console.error("Error fetching PJ Salta config:", error);
			setSnackbar({
				open: true,
				message: "Error cargando configuración PJ Salta",
				severity: "error",
			});
		} finally {
			setLoading(false);
	...
	const handleToggleWorker = async (workerType: "verifier" | "updater" | "stuck") => {
		...
		try {
			const result = await configPjSalta.toggleWorker(workerType);
			setWorkersData((prev) => {
				if (!prev) return prev;
				return {
					...prev,
	...
	const handleToggleManager = async () => {
		setActionLoading(true);
		try {
			const result = await configPjSalta.toggleManager();
			setWorkersData((prev) =>
				prev
					? {
							...prev,
	...
	const handleSaveWorkerConfig = async (updates: Partial<IManagerWorkerConfig>) => {
		...
		setActionLoading(true);
		try {
			await configPjSalta.updateWorkerConfig(editDialog.workerType, updates);
			setSnackbar({
				open: true,
	...
	const handleAcknowledgeAlert = async (alertId: string) => {
		try {
			await configPjSalta.acknowledgeAlert(alertId);
					...
					>
						<Stack spacing={1.5}>
							...
							<Typography variant="body2" color="text.secondary">
								Gestión de workers para el sistema PJ Salta (portal IOL del Poder Judicial de Salta) — Verificación, actualización y
				...
				<DialogContent>
					<Stack spacing={2} sx={{ pt: 1 }}>
						<Alert severity="info" variant="outlined">
							El cambio se guarda inmediatamente, pero requiere reiniciar el proceso PM2 <code>pjsalta-workers-manager</code> en producción
							para aplicar.
						</Alert>
						<CronSelector
							label="Frecuencia"
...
export default PjSaltaWorkersConfig;
