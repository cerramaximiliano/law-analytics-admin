...
const CausaDetalleModalPjSalta = ({ open, onClose, causa, onCausaUpdated }: CausaDetalleModalPjSaltaProps) => {
	...
	const [editedCausa, setEditedCausa] = useState<Partial<CausaPjSalta>>({});
	const [isSaving, setIsSaving] = useState(false);

	// Estados para la lista de movimientos
	const [currentMovimientos, setCurrentMovimientos] = useState<MovimientoPjSalta[]>([]);
	...
	const handleSaveEdit = async () => {
		try {
			...
			const response = await CausasPjSaltaService.updateCausa(causaId, dataToUpdate);

			if (response.success) {
				enqueueSnackbar("Causa PJ Salta actualizada correctamente", {
					variant: "success",
			...
			enqueueSnackbar("Error al actualizar la causa PJ Salta", {
				variant: "error",
				anchorOrigin: { vertical: "bottom", horizontal: "right" },
			});
			console.error(error);
			...
			<Dialog open={open} onClose={onClose} maxWidth="lg" fullWidth>
				...
				<Box sx={{ borderBottom: 1, borderColor: "divider" }}>
					<Tabs
						...
						aria-label="causa pjsalta detail tabs"
					>
...
export default CausaDetalleModalPjSalta;
