...
const getTodayUpdateInfo = (causa: CausaPjSalta): { isToday: boolean; count: number; hours: number[] } => {
...
const CarpetasVerificadasPjSalta = () => {
	const theme = useTheme();
	const { enqueueSnackbar } = useSnackbar();

	// Estados
	const [causas, setCausas] = useState<CausaPjSalta[]>([]);
	...
	// Modal de detalles
	const [selectedCausa, setSelectedCausa] = useState<CausaPjSalta | null>(null);
	...
	const fetchCausas = async (
		...
		try {
			...
			const response = await CausasPjSaltaService.getVerifiedCausas(params);
			...
			enqueueSnackbar("Error al cargar las carpetas verificadas PJ Salta", {
				variant: "error",
	...
	const handleVerDetalles = async (causa: CausaPjSalta) => {
		try {
			setLoadingDetail(true);
			const causaId = getId(causa._id);

			const response = await CausasPjSaltaService.getCausaById(causaId);
	...
	return (
		<MainCard title="Carpetas verificadas PJ Salta (App)" secondary={<CrossViewLinks current="datos" to={{ worker: "/admin/pjsalta/workers?worker=actualizacion" }} />}>
			...
			<Grid container spacing={3}>
				...
				<Grid item xs={12}>
					{loading ? (
						...
						<Alert severity="info">No se encontraron carpetas verificadas PJ Salta con los filtros seleccionados</Alert>
					) : (
						<Card>
							<TableContainer>
								<Table>
			...
			{/* Modal de detalles PJ Salta */}
			<CausaDetalleModalPjSalta open={detailModalOpen} onClose={handleCloseModal} causa={selectedCausa} onCausaUpdated={handleRefresh} />
		</MainCard>
	);
};

export default CarpetasVerificadasPjSalta;
