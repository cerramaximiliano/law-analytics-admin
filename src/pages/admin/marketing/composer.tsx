...
const EmailComposer = () => {
	...
	useEffect(() => {
		...
		debounceRef.current = setTimeout(async () => {
			try {
				setSearching(true);
				setOptions(await emailComposerService.searchRecipients(q));
			} catch {
				setOptions([]);
			} finally {
				setSearching(false);
	...
	const fetchDrafts = useCallback(async () => {
		try {
			...
			setDrafts(await emailComposerService.listDrafts());
		} catch (e: any) {
			enqueueSnackbar(e?.message || "Error cargando borradores", { variant: "error" });
		} finally {
	...
	const fetchSent = useCallback(async () => {
		try {
			setLoadingSent(true);
			const res = await EmailLogsService.getEmailLogs({
			...
			enqueueSnackbar(e?.message || "Error cargando enviados", { variant: "error" });
		} finally {
	...
	const saveDraft = async () => {
		...
		try {
			...
			enqueueSnackbar(e?.message || "Error guardando borrador", { variant: "error" });
		} finally {
	...
	const deleteDraft = async (id: string) => {
		try {
			...
			enqueueSnackbar(e?.message || "Error eliminando", { variant: "error" });
		}
	};
	...
	const refreshPreview = useCallback(async () => {
		try {
			...
			enqueueSnackbar(e?.message || "Error en vista previa", { variant: "error" });
		} finally {
	...
	const canSend =
		recipients.length > 0 && invalidRecipients.length === 0 && !!subject.trim() && !!content.replace(/<(.|\n)*?>/g, "").trim();
	...
	const doSend = async () => {
		...
		try {
			...
			if (res.success) {
				...
				enqueueSnackbar(`Enviados ${res.sent}/${res.total}${failed > 0 ? ` (${failed} fallidos)` : ""}`, {
						...
						<Paper variant="outlined" sx={{ p: { xs: 2, sm: 3 }, borderRadius: 2 }}>
							<Stack spacing={2.5}>
								...
								<Autocomplete
									...
									value={recipients}
									onChange={(_, vals) =>
										setRecipients([...new Set(vals.map((v) => (typeof v === "string" ? v.trim().toLowerCase() : v.email.toLowerCase())))])
									}
										...
										value.map((opt, index) => {
											...
											return (
												<Chip
													...
													label={email}
													size="small"
													color={!EMAIL_RE.test(email) ? "error" : "default"}
												/>
											);
										})
									}
								...
								<Box sx={{ bgcolor: "action.hover", borderRadius: 1, p: 1.25 }}>
									<Typography variant="caption" color="text.secondary">
										Personalización: usá <code>{"{nombre}"}</code>, <code>{"{nombreCompleto}"}</code> o <code>{"{email}"}</code> en el
										asunto, contenido o firma. Se reemplazan por cada destinatario (el nombre se busca por su email en usuarios/contactos).
									</Typography>
								</Box>
								<TextField
									size="small"
						...
						<Table size="small">
							...
							<TableBody>
								{loadingDrafts ? (
									...
									<TableRow>
										<TableCell colSpan={5} align="center" sx={{ py: 4 }}>
											<Typography color="text.secondary">No hay borradores</Typography>
										</TableCell>
									</TableRow>
								) : (
									drafts.map((d) => (
										<TableRow key={d._id} hover>
											...
											<TableCell align="right">
												<Button size="small" variant="outlined" onClick={() => loadDraft(d)} sx={{ mr: 1 }}>
													Cargar
												</Button>
												<Tooltip title="Eliminar">
													<IconButton size="small" color="error" onClick={() => deleteDraft(d._id)}>
														<Trash size={18} />
													</IconButton>
												</Tooltip>
											</TableCell>
			...
			{tab === 2 && (
				<Paper variant="outlined" sx={{ borderRadius: 2 }}>
					<Stack direction="row" justifyContent="flex-end" sx={{ p: 1 }}>
						<Button size="small" onClick={fetchSent} disabled={loadingSent}>
							Refrescar
						</Button>
					</Stack>
					<TableContainer>
						<Table size="small">
							...
							<TableBody>
								{loadingSent ? (
									...
									<TableRow>
										<TableCell colSpan={4} align="center" sx={{ py: 4 }}>
											<Typography color="text.secondary">Sin envíos todavía</Typography>
										</TableCell>
									</TableRow>
								) : (
									sentLogs.map((log) => (
										<TableRow key={log._id} hover>
											...
											<TableCell align="center">
												<Chip size="small" label={log.status} color={statusColor[log.status] || "default"} />
											</TableCell>
											<TableCell>
