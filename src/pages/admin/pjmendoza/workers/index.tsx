/**
 * PJ Mendoza Workers Configuration Page
...
import { CausasPjMendozaService, PipelineStatsResponse } from "api/causasPjMendoza";
import { BRAND_BLUE, LIVE_GREEN, STALE_AMBER, LIVE_PULSE_KEYFRAMES } from "themes/dashboardTokens";
import configPjMendoza, {
	IAllWorkersResponse,
...
const WorkerCard: React.FC<WorkerCardProps> = ({ workerType, config, status, effectiveSchedule, onToggle, onEdit, loading }) => {
	...
	const workerLabels: Record<string, { name: string; description: string; icon: React.ReactNode }> = {
		verifier: {
			name: "Verificación",
			description: "Verifica que los expedientes existen en el sistema PJ Mendoza",
...
const PjMendozaWorkersConfig: React.FC = () => {
	...
	const fetchData = useCallback(async () => {
		try {
			const [workersResponse, statsData, alertsData, pipelineData] = await Promise.all([
				configPjMendoza.getAllWorkersConfig(),
				configPjMendoza.getTodaySummary(),
				configPjMendoza.getAlerts(false),
				// Tolerante: el resto del dashboard no depende de esta llamada.
				CausasPjMendozaService.getPipelineStats().catch(() => null),
			]);
			...
			console.error("Error fetching PJ Mendoza config:", error);
			setSnackbar({
				open: true,
				message: "Error cargando configuración PJ Mendoza",
				severity: "error",
			});
		} finally {
			setLoading(false);
	...
	const handleToggleWorker = async (workerType: "verifier" | "updater" | "stuck") => {
		...
		try {
			const result = await configPjMendoza.toggleWorker(workerType);
			setWorkersData((prev) => {
				if (!prev) return prev;
				return {
					...prev,
	...
	const handleToggleManager = async () => {
		setActionLoading(true);
		try {
			const result = await configPjMendoza.toggleManager();
			setWorkersData((prev) =>
				prev
					? {
							...prev,
	...
	const handleSaveManagerCron = async (newCron: string) => {
		setActionLoading(true);
		try {
			await configPjMendoza.updateGlobalSettings({ managerCron: newCron.trim() });
			setSnackbar({
				open: true,
	...
	const handleSaveWorkerConfig = async (updates: Partial<IManagerWorkerConfig>) => {
		...
		setActionLoading(true);
		try {
			await configPjMendoza.updateWorkerConfig(editDialog.workerType, updates);
			setSnackbar({
				open: true,
	...
	const handleAcknowledgeAllAlerts = async () => {
		setActionLoading(true);
		try {
			const r = await configPjMendoza.acknowledgeAllAlerts();
			setAlerts([]);
			...
			<Grid container spacing={3}>
				...
				<Grid item xs={12}>
					<MainCard
						...
						secondary={
							<Stack direction="row" spacing={1} alignItems="center">
								<CrossViewLinks current="worker" to={{ datos: "/admin/pjmendoza/verified-app" }} />
								<Box
									sx={{
										display: "inline-flex",
										alignItems: "center",
					...
					>
						<Stack spacing={1.5}>
							<Stack direction="row" spacing={0.75} flexWrap="wrap" useFlexGap alignItems="center">
								...
								<Chip
									label="causas-pjmendoza · Atlas"
									size="small"
									color="info"
									variant="outlined"
									sx={{ fontFamily: "monospace", fontSize: "0.72rem" }}
								/>
								<Tooltip title="API REST en server principal, PM2 process: pjmendoza-api">
									<Chip
										label="pjmendoza.lawanalytics.app · API"
										size="small"
										color="default"
										variant="outlined"
										sx={{ fontFamily: "monospace", fontSize: "0.72rem" }}
							...
							<Typography variant="body2" color="text.secondary">
								Gestión de workers para el sistema PJ Mendoza (portal IOL del Poder Judicial de Salta) — Verificación, actualización y
				...
				<DialogContent>
					<Stack spacing={2} sx={{ pt: 1 }}>
						<Alert severity="info" variant="outlined">
							El cambio se guarda inmediatamente, pero requiere reiniciar el proceso PM2 <code>pjmendoza-workers-manager</code> en producción
							para aplicar.
						</Alert>
						<CronSelector
							label="Frecuencia"
...
export default PjMendozaWorkersConfig;
