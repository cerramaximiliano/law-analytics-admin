...
const getTodayUpdateInfo = (causa: CausaPjMendoza): { isToday: boolean; count: number; hours: number[] } => {
...
const CarpetasVerificadasPjMendoza = () => {
	const theme = useTheme();
	const { enqueueSnackbar } = useSnackbar();

	// Estados
	const [causas, setCausas] = useState<CausaPjMendoza[]>([]);
	...
	// Modal de detalles
	const [selectedCausa, setSelectedCausa] = useState<CausaPjMendoza | null>(null);
	...
	const fetchCausas = async (
		...
		try {
			...
			const response = await CausasPjMendozaService.getVerifiedCausas(params);
			...
			enqueueSnackbar("Error al cargar las carpetas verificadas PJ Mendoza", {
				variant: "error",
	...
	const handleVerDetalles = async (causa: CausaPjMendoza) => {
		try {
			setLoadingDetail(true);
			const causaId = getId(causa._id);

			const response = await CausasPjMendozaService.getCausaById(causaId);
	...
	return (
		<MainCard title="Carpetas verificadas PJ Mendoza (App)">
			...
			<Grid container spacing={3}>
				...
				<Grid item xs={12}>
					{loading ? (
						...
						<Alert severity="info">No se encontraron carpetas verificadas PJ Mendoza con los filtros seleccionados</Alert>
					) : (
						<Card>
							<TableContainer>
								<Table>
			...
			{/* Modal de detalles PJ Mendoza */}
			<CausaDetalleModalPjMendoza open={detailModalOpen} onClose={handleCloseModal} causa={selectedCausa} onCausaUpdated={handleRefresh} />
		</MainCard>
	);
};

export default CarpetasVerificadasPjMendoza;
