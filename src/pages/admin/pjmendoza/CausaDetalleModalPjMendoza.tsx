...
import { CausaPjMendoza, CausasPjMendozaService, MovimientoPjMendoza, IntervinientePjMendoza, CausaRelacionada, UpdateHistoryEntry } from "api/causasPjMendoza";
import { CloseCircle, CloseSquare, TickCircle, ArrowDown2, ArrowUp2, Copy, Edit, Save2, Lock1, Repeat } from "iconsax-react";
import { useSnackbar } from "notistack";

interface CausaDetalleModalPjMendozaProps {
	open: boolean;
	onClose: () => void;
	causa: CausaPjMendoza | null;
	onCausaUpdated?: () => void;
}

const CausaDetalleModalPjMendoza = ({ open, onClose, causa, onCausaUpdated }: CausaDetalleModalPjMendozaProps) => {
	...
	const [editedCausa, setEditedCausa] = useState<Partial<CausaPjMendoza>>({});
	const [isSaving, setIsSaving] = useState(false);

	// Estados para la lista de movimientos
	const [currentMovimientos, setCurrentMovimientos] = useState<MovimientoPjMendoza[]>([]);
	...
	useEffect(() => {
		if (open && causa) {
			...
			setIsEditing(false);
			setEditedCausa({});
			// El campo real en pjmendoza es `movimiento` (singular). `movimientos` es alias compat.
	...
	const handleSaveEdit = async () => {
		try {
			...
			const response = await CausasPjMendozaService.updateCausa(causaId, dataToUpdate);

			if (response.success) {
				enqueueSnackbar("Causa PJ Mendoza actualizada correctamente", {
					variant: "success",
			...
			enqueueSnackbar("Error al actualizar la causa PJ Mendoza", {
				variant: "error",
				anchorOrigin: { vertical: "bottom", horizontal: "right" },
			});
			console.error(error);
		...
		<>
			<Dialog open={open} onClose={onClose} maxWidth="lg" fullWidth>
					...
					<Stack direction="row" justifyContent="space-between" alignItems="center" spacing={2}>
						<Box sx={{ flex: 1 }}>
							<Stack direction="row" spacing={1} alignItems="center" sx={{ mb: 0.5 }}>
								...
								<Chip label="PJ Mendoza" color="primary" size="small" />
				...
				<Box sx={{ borderBottom: 1, borderColor: "divider" }}>
					<Tabs value={activeTab} onChange={handleTabChange} aria-label="causa pjmendoza detail tabs">
				...
				<DialogContent dividers sx={{ height: "500px", overflowY: "auto" }}>
					...
					{activeTab === 0 && (
						<Grid container spacing={2}>
							...
							{/* Sala y TribunalSuperior — no aplican a PJ Mendoza (campos legacy del modal eje) */}
							...
							{/* Errores */}
							{(causa.lastError || (causa.errorCount ?? 0) > 0) && (
								<>
									<Grid item xs={12} sx={{ mt: 1 }}>
								...
								<>
									<TableContainer>
										<Table size="small">
											...
											<TableBody>
												{paginatedMovimientos.map((mov: MovimientoPjMendoza, index: number) => (
							...
							{causa.intervinientes && causa.intervinientes.length > 0 ? (
								<TableContainer>
									<Table size="small">
										...
										<TableBody>
											{causa.intervinientes.map((interv: IntervinientePjMendoza, index: number) => (
...
export default CausaDetalleModalPjMendoza;
