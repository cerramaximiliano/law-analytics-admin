...
const LinkedCausasRow = ({
	...
	pivot: CausaPjMendoza;
	expanded: boolean;
	onSelectCausa: (pivotId: string, causaId: string, causa: CausaPjMendoza) => void;
}) => {
	const [linkedCausas, setLinkedCausas] = useState<CausaPjMendoza[]>([]);
	...
	const loadLinkedCausas = async () => {
		try {
			setLoading(true);
			const response = await CausasPjMendozaService.getPivotLinkedCausas(getId(pivot._id));
			if (response.success) {
...
const CarpetasPivotsPjMendoza = () => {
	const theme = useTheme();
	const { enqueueSnackbar } = useSnackbar();

	// Estados
	const [pivots, setPivots] = useState<CausaPjMendoza[]>([]);
	const [loading, setLoading] = useState(false);
	...
	const fetchPivots = async (
		...
		try {
			...
			const response = await CausasPjMendozaService.getPivotCausas(params);
			...
			enqueueSnackbar("Error al cargar los pivots PJ Mendoza", {
				variant: "error",
	...
	const handleSelectCausa = (pivotId: string, causaId: string, causa: CausaPjMendoza) => {
		setConfirmDialog({
			open: true,
			pivotId,
			causaId,
	...
	return (
		<MainCard title="Pivots PJ Mendoza — múltiples resultados">
			{/* Header: Resultados */}
			<Box sx={{ mb: 2 }}>
				<Grid container spacing={2} alignItems="center">
					...
					<Grid item xs={12} md={8}>
						<Alert severity="info" icon={<DocumentText size={20} />}>
							Los pivots son búsquedas que encontraron múltiples resultados en PJ Mendoza. Expande cada fila para ver las opciones y seleccionar la
							causa correcta.
						</Alert>
					</Grid>
				</Grid>
...
export default CarpetasPivotsPjMendoza;
