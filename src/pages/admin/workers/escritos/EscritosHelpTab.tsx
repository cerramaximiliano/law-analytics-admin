...
const Node: React.FC<NodeProps> = ({ title, subtitle, items, color, badge, badgeColor }) => {
	const theme = useTheme();
	return (
		<Box sx={{ position: "relative", border: `2px solid ${color}`, borderRadius: 2, p: 2, bgcolor: alpha(color, 0.04), minWidth: 190, flex: 1 }}>
			{badge && (
				<Box sx={{ position: "absolute", top: -10, left: 12, bgcolor: badgeColor || color, color: "#fff", px: 1, py: 0.2, borderRadius: 1, fontSize: "0.62rem", fontWeight: 700, letterSpacing: 0.5 }}>
					{badge}
				</Box>
			)}
			<Typography variant="subtitle2" fontWeight={700} sx={{ color }}>{title}</Typography>
			{subtitle && <Typography variant="caption" color="text.secondary" display="block" sx={{ mb: 0.5 }}>{subtitle}</Typography>}
			<Stack spacing={0.3} sx={{ mt: 0.5 }}>
				{items.map((item, i) => (
					<Typography key={i} variant="caption" color="text.secondary" sx={{ pl: 1, borderLeft: `2px solid ${alpha(color, 0.3)}` }}>{item}</Typography>
				))}
			</Stack>
		</Box>
	);
	...
	return (
		<Stack alignItems="center" justifyContent="center" sx={{ px: 0.5, minWidth: 44 }}>
			<Box sx={{ display: "flex", alignItems: "center" }}>
				...
				<Box sx={{ width: 0, height: 0, borderTop: "5px solid transparent", borderBottom: "5px solid transparent", borderLeft: `7px solid ${theme.palette.divider}` }} />
			</Box>
			{label && <Typography variant="caption" color="text.secondary" sx={{ fontSize: "0.6rem", mt: 0.4, whiteSpace: "nowrap", textAlign: "center" }}>{label}</Typography>}
		</Stack>
	);
};
...
const ArrowDown: React.FC<{ label?: string }> = ({ label }) => {
	...
	return (
		<Stack alignItems="center" sx={{ py: 0.5 }}>
			...
			<Box sx={{ width: 0, height: 0, borderLeft: "5px solid transparent", borderRight: "5px solid transparent", borderTop: `7px solid ${theme.palette.divider}` }} />
			{label && <Typography variant="caption" color="text.secondary" sx={{ fontSize: "0.62rem", fontStyle: "italic", mt: 0.3 }}>{label}</Typography>}
		</Stack>
	);
};

const SectionTitle: React.FC<{ children: React.ReactNode }> = ({ children }) => (
	<Typography variant="subtitle1" fontWeight={700} sx={{ mb: 1.5, mt: 1 }}>{children}</Typography>
);

const InfoRow: React.FC<{ label: string; value: React.ReactNode }> = ({ label, value }) => (
	<Box sx={{ display: "flex", gap: 2, py: 0.6, borderBottom: "1px solid", borderColor: "divider" }}>
		<Typography variant="caption" color="text.secondary" sx={{ minWidth: 175, fontWeight: 600 }}>{label}</Typography>
		...
		<Stack spacing={3}>
			<Box>
				<Typography variant="body2" color="text.secondary">
					...
					buscando escritos nuevos, descarga sus PDFs, extrae el texto, lo divide en fragmentos semánticos (chunks),
					genera embeddings y los almacena en Pinecone. En una segunda fase calcula un <strong>puntaje de novedad</strong> comparando
					cada escrito contra el corpus existente para identificar planteos jurídicos inusuales.
				</Typography>
			</Box>
			...
			<Box>
				...
				<Box sx={{ border: "1px solid", borderColor: "divider", borderRadius: 2, overflow: "hidden" }}>
					<InfoRow label="Servidor" value={<><strong>worker_01</strong> · IP 100.111.73.56</>} />
			...
			<Box>
				...
				<Box sx={{ border: "1px solid", borderColor: "divider", borderRadius: 2, p: 2, bgcolor: "background.paper" }}>
					...
					<Stack direction="row" alignItems="stretch" spacing={1} sx={{ mb: 1 }}>
						<Node badge="SERVIDOR LOCAL" badgeColor={colors.local} title="MongoDB Local" subtitle="worker_01 · 100.111.73.56" items={["CausasCivil (fuero CIV)", "CausasTrabajo (fuero CNT)", "CausasSegSoc (fuero CSS)", "CausasComercial (fuero COM)", "movimiento.url · movimiento.detalle"]} color={colors.local} />
						<Arrow label="scan.job.js (cron)" />
						<Node badge="SERVIDOR LOCAL" badgeColor={colors.redis} title="Redis (BullMQ)" subtitle="worker_01 · puerto 6379" items={["Cola: escritos-extract", "Cola: escritos-select", "jobId determinístico (dedup)", "Reintentos: 3x / 2x"]} color={colors.redis} />
					</Stack>
					...
					<Stack direction="row" alignItems="stretch" spacing={1} sx={{ mb: 1 }}>
						...
						<Stack spacing={1} sx={{ flex: 1 }}>
							...
							<Node badge="VECTOR DB" badgeColor={colors.pinecone} title="Pinecone" subtitle="pjn-style-corpus-v2 · global-chunks" items={["Upsert en batches de 100", "metadata: causeId, docType, sectionType, chunkText"]} color={colors.pinecone} />
						</Stack>
					</Stack>
					...
					{/* Row 3: novelty */}
					<Stack direction="row" alignItems="stretch" spacing={1} sx={{ mb: 1 }}>
						<Node badge="FASE 2" badgeColor={colors.worker} title="Selector Worker" subtitle="worker_01 · Novelty Detection" items={["Selecciona chunks (estrategia A/B)", "Fetch vectores de Pinecone", "Query vecinos (same-doctype)", "noveltyScore = 1 − mean(maxSim)", "Label: routine / review / alert"]} color={colors.worker} />
						<Arrow label="persiste" />
						<Node badge="CLOUD" badgeColor={colors.atlas} title="MongoDB Atlas" subtitle="global_documents (cloud)" items={["noveltyScore, noveltyLabel", "noveltyTopNeighbors (top 5)", "noveltyStatus: done/skipped/error", "noveltyAnalyzedAt"]} color={colors.atlas} />
					</Stack>
					...
					<Stack direction="row" alignItems="center" spacing={1}>
						...
						<Node badge="SERVIDOR LOCAL" badgeColor={colors.local} title="MongoDB Local" subtitle="worker_01 · movimiento actualizado" items={["movimiento.globalProcessed = true", "Evita re-procesar en próximo scan"]} color={colors.local} />
					</Stack>
			...
			<Box>
				...
				<Typography variant="body2" color="text.secondary">
					El worker opera con <strong>dos bases de datos separadas</strong>: la BD local de causas que alimentan los workers de scraping
					del PJN (fuente de datos), y MongoDB Atlas en la nube (destino de resultados enriquecidos con texto, embeddings y novedad).
					Pinecone es independiente de ambas y almacena únicamente los vectores para búsqueda semántica.
				</Typography>
		...
		<Stack spacing={3}>
				...
				<Box key={p.num} sx={{ border: `1px solid ${p.color}`, borderRadius: 2, overflow: "hidden" }}>
					<Box sx={{ bgcolor: alpha(p.color, 0.08), px: 2, py: 1.2, borderBottom: `1px solid ${alpha(p.color, 0.3)}` }}>
						<Stack direction="row" alignItems="center" spacing={1.5}>
							<Box sx={{ width: 28, height: 28, borderRadius: "50%", bgcolor: p.color, color: "#fff", display: "flex", alignItems: "center", justifyContent: "center", fontWeight: 700, fontSize: "0.85rem", flexShrink: 0 }}>{p.num}</Box>
							<Typography variant="subtitle1" fontWeight={700}>{p.title}</Typography>
						</Stack>
					</Box>
					<Box sx={{ px: 2, py: 1.5 }}>
						<Typography variant="body2" color="text.secondary" sx={{ mb: 1.5 }}>{p.desc}</Typography>
						<Stack spacing={0.4}>
							{p.steps.map((s, i) => (
								<Stack key={i} direction="row" spacing={1} alignItems="flex-start">
									<Box sx={{ width: 18, height: 18, borderRadius: "50%", bgcolor: alpha(p.color, 0.15), color: p.color, display: "flex", alignItems: "center", justifyContent: "center", fontSize: "0.6rem", fontWeight: 700, flexShrink: 0, mt: 0.1 }}>{i + 1}</Box>
			...
			<Box>
				...
				<Typography variant="body2" color="text.secondary" sx={{ mb: 1.5 }}>
					El PJN usa <code>movimiento.tipo</code> genérico ("ESCRITO AGREGADO") para todos los escritos de parte.
					El tipo real del documento está en <code>movimiento.detalle</code> (texto libre: "CONTESTA DEMANDA", "INICIA DEMANDA POR DAÑOS Y PERJUICIOS", etc.).
				</Typography>
				<Box sx={{ border: "1px solid", borderColor: "divider", borderRadius: 2, overflow: "hidden" }}>
					{[
						...
						<Box key={i} sx={{ display: "flex", gap: 2, py: 0.6, px: 1.5, borderBottom: i < 6 ? "1px solid" : "none", borderColor: "divider" }}>
							<Typography variant="caption" sx={{ minWidth: 200, fontFamily: "monospace", fontWeight: 600, color: colors.worker }}>{dtype}</Typography>
							<Typography variant="caption" color="text.secondary">{patterns}</Typography>
						</Box>
					))}
				</Box>
				<Typography variant="caption" color="text.secondary" sx={{ mt: 0.5, display: "block" }}>
					⚡ Solo los docTypes: demanda, contestacion_demanda, reconvencion, expresion_agravios, contestacion_agravios, recurso_extraordinario pasan a la Fase 2 (novelty detection).
				</Typography>
			</Box>
		</Stack>
	);
...
const NoveltySection: React.FC = () => {
	...
	return (
		<Stack spacing={3}>
			<Box>
				<Typography variant="body2" color="text.secondary">
					El <strong>noveltyScore</strong> mide qué tan diferente es un escrito respecto al corpus ya indexado.
					Un valor cercano a 0 indica que el escrito es muy similar a otros ya vistos (rutinario); cercano a 1 indica que es único en el corpus.
				</Typography>
			</Box>

			<Box sx={{ border: "1px solid", borderColor: "divider", borderRadius: 2, overflow: "hidden" }}>
				<Box sx={{ bgcolor: alpha(colors.pinecone, 0.06), px: 2, py: 1, borderBottom: "1px solid", borderColor: "divider" }}>
					<Typography variant="subtitle2" fontWeight={700}>Fórmula</Typography>
				</Box>
				<Box sx={{ px: 2, py: 1.5 }}>
					...
					<Typography variant="caption" color="text.secondary">
						Para cada chunk analizado se buscan los K vecinos más cercanos en Pinecone (de <em>otras</em> causas, mismo docType).
						Se toma el más cercano (maxSimilarity). El noveltyScore promedia esas distancias inversas.
					</Typography>
				</Box>
			</Box>
			...
			<Box>
				...
				<Stack spacing={1}>
					{[
						{ label: "routine", range: "score < 0.194", desc: "Escrito estándar, muy similar al corpus. Planteos frecuentes, lenguaje formulaico.", color: theme.palette.success.main },
						{ label: "review", range: "0.194 ≤ score < 0.234", desc: "Moderadamente novedoso (~25% del corpus). Merece revisión manual.", color: theme.palette.warning.main },
						{ label: "alert", range: "score ≥ 0.234", desc: "Muy novedoso (~10% del corpus). Sin precedentes claros en el corpus actual.", color: theme.palette.error.main },
					].map((item) => (
						<Box key={item.label} sx={{ border: `1px solid ${alpha(item.color, 0.4)}`, borderRadius: 2, p: 1.5, bgcolor: alpha(item.color, 0.04) }}>
							<Stack direction="row" spacing={1.5} alignItems="flex-start">
								<Box sx={{ px: 1, py: 0.3, bgcolor: item.color, color: "#fff", borderRadius: 1, fontSize: "0.7rem", fontWeight: 700, flexShrink: 0, mt: 0.3 }}>{item.label.toUpperCase()}</Box>
								<Box>
									<Typography variant="caption" fontWeight={700}>{item.range}</Typography>
				...
				<Typography variant="caption" color="text.secondary" sx={{ mt: 1, display: "block" }}>
					Los umbrales se ajustan desde Panel → Config (noveltyThresholdTrack / noveltyThresholdAlert) sin reiniciar el worker.
					Recalibrar cuando el corpus supere los 500 documentos por docType.
				</Typography>
			</Box>
				...
				<Stack spacing={1}>
					{[
						...
						<Box key={s.name} sx={{ border: "1px solid", borderColor: "divider", borderRadius: 2, p: 1.5 }}>
							...
							<Typography variant="caption" color="text.secondary" display="block" sx={{ mt: 0.3 }}>{s.desc}</Typography>
							<Typography variant="caption" color="primary.main" display="block" sx={{ mt: 0.3 }}>✓ Mejor para: {s.best}</Typography>
						</Box>
					))}
				</Stack>
			</Box>
			...
			<Box>
				...
				<Box sx={{ border: "1px solid", borderColor: "divider", borderRadius: 2, overflow: "hidden" }}>
					{[
						...
						["skipped", "Saltado: noveltyEnabled=false, docType no elegible, sin chunks sustantivos, ya analizado.", theme.palette.text.secondary],
						...
						<Box key={i as number} sx={{ display: "flex", gap: 2, py: 0.8, px: 1.5, borderBottom: i < 2 ? "1px solid" : "none", borderColor: "divider", alignItems: "flex-start" }}>
							<Box sx={{ px: 1, py: 0.2, bgcolor: alpha(color as string, 0.12), color: color as string, borderRadius: 1, fontSize: "0.68rem", fontWeight: 700, flexShrink: 0, mt: 0.2, fontFamily: "monospace" }}>{status}</Box>
							<Typography variant="caption" color="text.secondary">{desc as string}</Typography>
						</Box>
					))}
				</Box>
			</Box>
...
const TroubleSection: React.FC = () => {
	...
	const issues = [
		...
		},
		{
			...
			cause: "movimiento.tipo es siempre genérico en el PJN. El tipo real está en movimiento.detalle. Si detalle no matchea ningún patrón del DETALLE_MAP → null.",
	...
	return (
		<Stack spacing={2}>
			{issues.map((item, i) => (
				<Box key={i} sx={{ border: "1px solid", borderColor: "divider", borderRadius: 2, overflow: "hidden" }}>
					<Box sx={{ px: 2, py: 1, bgcolor: alpha(theme.palette.error.main, 0.06), borderBottom: "1px solid", borderColor: "divider" }}>
						<Typography variant="caption" fontWeight={700} color="error.main">Problema</Typography>
						<Typography variant="body2" fontWeight={600}>{item.prob}</Typography>
					</Box>
					<Box sx={{ px: 2, py: 1, borderBottom: "1px solid", borderColor: "divider" }}>
						<Typography variant="caption" fontWeight={700} color="text.secondary">Causa</Typography>
						<Typography variant="body2" color="text.secondary">{item.cause}</Typography>
					</Box>
					<Box sx={{ px: 2, py: 1 }}>
						<Typography variant="caption" fontWeight={700} color="success.main">Solución</Typography>
			...
			<Box sx={{ border: "1px solid", borderColor: "divider", borderRadius: 2, p: 2 }}>
				<Typography variant="subtitle2" fontWeight={700} sx={{ mb: 1 }}>Comandos útiles en worker_01</Typography>
				{[
					...
					<Box key={i} sx={{ mb: 0.8 }}>
						...
						<Box sx={{ bgcolor: "#f5f5f5", borderRadius: 1, px: 1.5, py: 0.5, fontFamily: "monospace", fontSize: "0.75rem", mt: 0.3 }}>{cmd}</Box>
					</Box>
				))}
			</Box>
		</Stack>
	...
	return (
		<Stack spacing={0}>
			<Box sx={{ borderBottom: 1, borderColor: "divider", mb: 3 }}>
				<Tabs value={sub} onChange={(_, v) => setSub(v)} variant="scrollable" scrollButtons="auto"
