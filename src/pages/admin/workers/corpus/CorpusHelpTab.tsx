...
const Node: React.FC<NodeProps> = ({ title, subtitle, items, color, badge, badgeColor }) => {
	const theme = useTheme();
	return (
		<Box sx={{ position: "relative", border: `2px solid ${color}`, borderRadius: 2, p: 2, bgcolor: alpha(color, 0.04), minWidth: 200, flex: 1 }}>
			{badge && (
...
const Arrow: React.FC<{ label?: string }> = ({ label }) => {
	...
	return (
		<Stack alignItems="center" justifyContent="center" sx={{ px: 1, minWidth: 40 }}>
			...
			<Box sx={{ width: 0, height: 0, borderTop: "5px solid transparent", borderBottom: "5px solid transparent", borderLeft: `7px solid ${theme.palette.divider}`, ml: "23px", mt: "-5px" }} />
			{label && <Typography variant="caption" color="text.secondary" sx={{ fontSize: "0.6rem", mt: 0.5, whiteSpace: "nowrap" }}>{label}</Typography>}
		</Stack>
	);
};
		...
		<Stack spacing={3}>
			<Box>
				<Typography variant="body2" color="text.secondary">
					El <strong>corpus de estilo jurídico</strong> es una colección curada de escritos judiciales reales usados como ejemplos
					de calidad por el asistente IA al generar o reformular documentos. No es un worker continuo: se construye con
					scripts manuales y se auto-alimenta cuando el pipeline RAG indexa nuevos expedientes.
				</Typography>
			</Box>
			...
			<Box>
				...
				<Box sx={{ border: "1px solid", borderColor: "divider", borderRadius: 2, overflow: "hidden" }}>
					<InfoRow label="Servidor" value={<><strong>worker_02</strong> · IP 100.98.180.101 · Ryzen 5700X · 32 GB RAM</>} />
					...
					<InfoRow label="Índice Pinecone" value={<><code style={{ background: "#f5f5f5", padding: "1px 5px", borderRadius: 3 }}>pjn-style-corpus</code> · 1024 dims · coseno · serverless</>} />
			...
			<Box>
				...
				<Typography variant="body2" color="text.secondary" sx={{ mb: 2 }}>
					Cuando el pipeline RAG indexa un expediente, el <code>ingester.js</code> inyecta automáticamente
					los escritos elegibles al corpus sin necesidad de correr scripts manuales.
				</Typography>
				<Box sx={{ overflowX: "auto", pb: 1 }}>
...
const ScriptsSection: React.FC = () => {
	...
	return (
		<Stack spacing={3}>
			<Typography variant="body2" color="text.secondary">
				El corpus se gestiona con scripts que se ejecutan manualmente desde <strong>worker_02</strong>.
				No hay un daemon ni cron que los corra automáticamente.
			</Typography>

			<Box sx={{ border: "1px solid", borderColor: "divider", borderRadius: 2, overflow: "hidden" }}>
				<Box sx={{ bgcolor: alpha(colors.script, 0.08), px: 2, py: 1, borderBottom: "1px solid", borderColor: "divider" }}>
					<Typography variant="subtitle2" fontWeight={700}>Directorio: pjn-rag-api/scripts/</Typography>
					<Typography variant="caption" color="text.secondary">worker_02 · /home/[usuario]/pjn-rag-api</Typography>
				</Box>
				{scripts.map((s, i) => (
					<Box key={i} sx={{ px: 2, py: 1.5, borderBottom: i < scripts.length - 1 ? "1px solid" : "none", borderColor: "divider" }}>
						<Typography variant="subtitle2" fontWeight={700} sx={{ color: colors.script, fontFamily: "monospace" }}>{s.name}</Typography>
						<Typography variant="caption" color="text.secondary" display="block" sx={{ my: 0.5 }}>{s.desc}</Typography>
						<Box sx={{ bgcolor: "#f5f5f5", borderRadius: 1, px: 1.5, py: 0.8, fontFamily: "monospace", fontSize: "0.75rem", mb: 0.5 }}>
							{s.usage}
						</Box>
						<Stack direction="row" spacing={2} flexWrap="wrap">
							<Typography variant="caption" color="text.secondary"><strong>Cuándo:</strong> {s.when}</Typography>
							<Typography variant="caption" color="text.secondary"><strong>Servidor:</strong> {s.server}</Typography>
						</Stack>
					</Box>
				))}
			</Box>
			...
			<Box>
				...
				<Stack alignItems="center" spacing={0}>
					<Node badge="PASO 1" color={colors.mongo} title="RagDocument (Atlas)" subtitle="Escritos ya indexados por el pipeline RAG" items={["movimientoTipo = ESCRITO AGREGADO", "textLength > 400 chars", "fullText disponible"]} />
					<ArrowDown label="build-style-corpus.js" />
					<Node badge="PASO 2" color={colors.script} title="LegalStyleCorpus (Atlas)" subtitle="Colección de ejemplos jurídicos" items={["textPreview = primeros 800 chars", "quality = high (si tiene saludo a juez) / normal", "vectorId = null (pendiente de embed)"]} />
					<ArrowDown label="embed-style-corpus.js (solo quality=high)" />
					<Node badge="PASO 3" color={colors.pinecone} title="pjn-style-corpus (Pinecone)" subtitle="Índice vectorial compartido" items={["ID = _id MongoDB del corpus doc", "Vector 1024 dims (text-embedding-3-small)", "Metadata: fuero, title, textPreview (600 chars)"]} />
					<ArrowDown label="a partir de aquí disponible" />
					<Node badge="PRODUCCIÓN" color={colors.openai} title="Editor IA (pjn-rag-api)" subtitle="few-shot retrieval en tiempo real" items={["Query semántica por fuero", "Top-K vecinos como ejemplos", "Inyectados al prompt de GPT-4o"]} />
				</Stack>
			</Box>
		</Stack>
	);
...
const TroubleSection: React.FC = () => {
	...
	const issues = [
		...
		},
		{
			...
			cause: "No hay RagDocuments con movimientoTipo='ESCRITO AGREGADO' y textLength>400 en Atlas, o ya todos tienen entrada en LegalStyleCorpus.",
		...
		},
		{
			...
			cause: "El ingester.js solo inyecta calidad high. Si el escrito no tiene saludo a juez, queda como normal y no se embeds automáticamente.",
