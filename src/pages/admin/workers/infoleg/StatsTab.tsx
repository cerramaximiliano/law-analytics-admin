...
import {
  ...
  Grid, Skeleton, Stack, Table, TableBody, TableCell,
...
const StatsTab = () => {
  ...
  const pctScraped = stats.totals.total > 0
    ? ((stats.totals.scraped / stats.totals.total) * 100).toFixed(1)
    : '0';
  ...
  return (
    <Stack spacing={3}>
      ...
      <Box>
        ...
        <Grid container spacing={2}>
          <Grid item xs={12} sm={6} md={3}>
            <StatCard label="Total scrapeado" value={stats.totals.scraped}
              sub={`${pctScraped}% del total`} color="success" icon={<TickCircle size={22} />} />
          </Grid>
          <Grid item xs={12} sm={6} md={3}>
            <StatCard label="Pendientes" value={stats.totals.pending}
              color="warning" icon={<DocumentText size={22} />} />
          </Grid>
          <Grid item xs={12} sm={6} md={3}>
            <StatCard label="No encontrados (404)" value={stats.totals.not_found}
              color="info" />
          </Grid>
          <Grid item xs={12} sm={6} md={3}>
            <StatCard label="Con error" value={stats.totals.error}
              color="error" icon={<Warning2 size={22} />} />
          </Grid>
        </Grid>
      </Box>
      ...
      {/* Tasks */}
      <Box>
        <Typography variant="h6" gutterBottom>Cobertura de contenido</Typography>
        <Grid container spacing={2}>
          <Grid item xs={12} sm={6} md={3}>
      ...
      {stats.scraping && (
        <Box>
          <Typography variant="h6" gutterBottom>Rango de scraping</Typography>
          <Grid container spacing={2}>
      ...
      {stats.porTipo.length > 0 && (
        <Box>
          <Typography variant="h6" gutterBottom>Por tipo de norma</Typography>
      ...
      {stats.recientes.length > 0 && (
        <Box>
          <Typography variant="h6" gutterBottom>Últimas normas scrapeadas</Typography>
          <TableContainer sx={{ borderRadius: 1.5, border: '1px solid', borderColor: 'divider' }}>
            <Table size="small">
              <TableHead>
                <TableRow>
                  ...
                  <TableCell sx={{ fontWeight: 600 }} align="right">Scrapeado</TableCell>
                </TableRow>
              </TableHead>
              <TableBody>
