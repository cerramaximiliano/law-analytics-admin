...
import {
  ...
  DialogContent, DialogTitle, Divider, Grid, IconButton, Skeleton,
    ...
    <Dialog open={open} onClose={onClose} maxWidth="xs" fullWidth>
      ...
      <DialogContent sx={{ pt: 2 }}>
        <Typography variant="body2" color="text.secondary" sx={{ mb: 2 }}>
          Crea documentos <code>pending</code> en la colección para el rango indicado.
          Los IDs ya existentes no se modifican.
        </Typography>
        <Grid container spacing={2}>
          <Grid item xs={6}>
            <TextField fullWidth label="Desde (ID)" type="number" value={fromId}
              onChange={e => setFromId(e.target.value)} size="small" />
          </Grid>
          <Grid item xs={6}>
            <TextField fullWidth label="Hasta (ID)" type="number" value={toId}
              onChange={e => setToId(e.target.value)} size="small" />
          </Grid>
        </Grid>
...
const WorkerStatusTab = () => {
  ...
  const statusCfg = (status: string) =>
    STATUS_CONFIG[status] || { color: 'default' as const, label: status };
  ...
  return (
    <Stack spacing={3}>
      ...
      {loading && workers.length === 0 ? (
        <Stack spacing={1}>
          {[0, 1, 2].map(i => <Skeleton key={i} variant="rounded" height={60} />)}
        </Stack>
      ) : (
          ...
          <Table size="small">
            <TableHead>
              <TableRow sx={{ bgcolor: alpha(theme.palette.primary.main, 0.04) }}>
                ...
                <TableCell sx={{ fontWeight: 600 }} align="right">Memoria</TableCell>
                <TableCell sx={{ fontWeight: 600 }} align="right">Uptime</TableCell>
                <TableCell sx={{ fontWeight: 600 }} align="right">Reinicios</TableCell>
      ...
      <Divider />
      <Typography variant="caption" color="text.secondary">
        Estado actualizado automáticamente cada 15 segundos.
        Los workers son gestionados por PM2 — el manager puede escalarlos automáticamente
        según el volumen de normas pendientes.
      </Typography>
