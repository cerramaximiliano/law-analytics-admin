// Respaldo en la nube del scraping del portal autenticado del PJN.
//
// El proceso `pjn-mis-causas-manager` late un heartbeat cada pocos minutos; si
// deja de latir, una Lambda levanta workers en ECS Fargate y toma el trabajo
// hasta que el box vuelve. Es el único servicio del ecosistema con respaldo
// automático, y por eso el panel vive dentro de la pestaña del box donde ese
// proceso corre: worker-cloud-02 (antes worker_02, migrado el 2026-08-24).
...
/** Box donde corre hoy el manager que sostiene el heartbeat. */
const PRIMARY_BOX = "worker-cloud-02";
...
const FailoverPanel = () => {
	...
	return (
		<Grid container spacing={2.5}>
			<Grid item xs={12}>
				...
				>
					{draining ? (
						<>
							{liveDot(STALE_AMBER)}
							<strong>CLOUD DRENANDO — {PRIMARY_BOX} recuperado</strong>
						...
						<>
							<strong>FAILOVER ACTIVO — el scraping corre en ECS Fargate</strong>
						...
						<>
							{liveDot(LIVE_GREEN)}
							<strong>{PRIMARY_BOX} ACTIVO — funcionamiento normal</strong>
						</>
					)}
					{lastRefresh && (
					...
					<Stack spacing={1} alignItems="flex-start">
						<Stack direction="row" spacing={1} alignItems="center">
							...
							<Typography variant="subtitle2" color="text.secondary">
								Ejecutando el scraping
							</Typography>
						</Stack>
						<Chip label={cloudActive ? "ECS Fargate (respaldo)" : PRIMARY_BOX} color={cloudActive ? "error" : "success"} size="small" />
						{status?.updatedAt && (
				...
				<MainCard>
					<Stack spacing={1} alignItems="flex-start">
						<Stack direction="row" spacing={1} alignItems="center">
							<Timer1 size={20} />
							<Typography variant="subtitle2" color="text.secondary">
								Heartbeat de pjn-mis-causas-manager
							</Typography>
						</Stack>
						{status?.heartbeat.msSinceLastPoll !== null && status?.heartbeat.msSinceLastPoll !== undefined ? (
							<>
								<Chip
			...
			{/* Tasks ECS: solo existen mientras el respaldo está tomado */}
			{cloudActive && (
				<Grid item xs={12}>
					<MainCard
						title={
