...
import {
  ...
  Divider, Grid, IconButton, Stack, Tab, Tabs, Tooltip,
...
const TIPO_LABELS: Record<string, string> = {
  ley: 'Ley', decreto: 'Decreto', decreto_ley: 'Decreto-Ley',
  decreto_reglamentario: 'D. Reglamentario', decreto_dnu: 'DNU',
  resolucion: 'Resolución', resolucion_general: 'Res. General',
  resolucion_administrativa: 'Res. Administrativa',
  decision_administrativa: 'Dec. Administrativa',
  disposicion: 'Disposición', disposicion_conjunta: 'Disp. Conjunta',
...
interface MetaRowProps { label: string; value?: string | React.ReactNode }
    ...
    <Dialog open={open} onClose={onClose} maxWidth="md" fullWidth
      ...
      <DialogTitle sx={{ pb: 1 }}>
        <Stack direction="row" justifyContent="space-between" alignItems="center">
          <Stack direction="row" spacing={1.5} alignItems="center">
            {norma && (
              <Chip
                label={TIPO_LABELS[norma.tipo || ''] || norma.tipo || '?'}
                size="small" color="primary" variant="outlined"
              />
            )}
            <Typography variant="h5">
              {norma
                ? `${norma.numero ? `N° ${norma.numero}` : ''} ${norma.anio ? `(${norma.anio})` : ''}`.trim()
                : `ID ${infolegId}`}
            </Typography>
          </Stack>
          <Stack direction="row" spacing={0.5}>
            ...
            <IconButton size="small" onClick={onClose}><CloseCircle size={20} /></IconButton>
          </Stack>
        </Stack>
      </DialogTitle>
      ...
      <Box sx={{ borderBottom: 1, borderColor: 'divider', px: 3 }}>
        <Tabs value={tab} onChange={(_, v) => setTab(v)}
          ...
          <Tab value="texto" label={`Texto original${!hasTextoOriginal ? ' (N/D)' : ''}`}
            disabled={!hasTextoOriginal} />
          <Tab value="texact" label={`Texto actualizado${!hasTextoActualizado ? ' (N/D)' : ''}`}
            disabled={!hasTextoActualizado} />
          <Tab value="vinc" label={`Vinculaciones${hasVinculaciones ? ` (${norma?.vinculacionesRaw?.length})` : ''}`}
              ...
              <Stack spacing={2.5}>
                <Box>
                  ...
                  <Stack direction="row" spacing={1} flexWrap="wrap" gap={0.5}>
                    ...
                    {norma.tasks?.textoActualizadoStale && (
                      <Chip label="Texto desactualizado" color="warning" size="small" />
                    )}
                  </Stack>
                </Box>
                ...
                <Grid container spacing={1.5}>
                  ...
                  <MetaRow label="Fecha publicación" value={norma.fechaPublicacion ? dayjs(norma.fechaPublicacion).format('DD/MM/YYYY') : undefined} />
                  <MetaRow label="Boletín Oficial"   value={norma.boletin?.numero ? `N° ${norma.boletin.numero}${norma.boletin.fecha ? ` — ${dayjs(norma.boletin.fecha).format('DD/MM/YYYY')}` : ''}` : undefined} />
                  ...
                  <>
                    ...
                    <Box>
                      ...
                      <Stack direction="row" spacing={1} flexWrap="wrap" gap={0.5}>
                        ...
                        ].filter(([k]) => (norma.vinculacionesEntrantes as any)[k] > 0).map(([k, label]) => (
                          <Chip
                            key={k}
                ...
                {norma.vinculacionesRaw.map((v, i) => (
                  <Box key={i} sx={{
                    ...
                    <Stack direction="row" spacing={1.5} alignItems="flex-start">
                      <Chip label={v.tipo} size="small" variant="outlined" color="primary"
