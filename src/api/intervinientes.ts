...
export class IntervinientesService {
	 ...
	 */
	static async getByExpediente(fuero: string, number: number, year: number): Promise<IntervinientesResponse> {
		try {
			const response = await pjnAxios.get(`/api/intervinientes/expediente/${fuero}/${number}/${year}`);
			return response.data;
		} catch (error) {
			throw this.handleError(error);
		}
	 ...
	 */
	static async searchByNombre(nombre: string, page: number = 1, limit: number = 20): Promise<IntervinientesListResponse> {
		try {
