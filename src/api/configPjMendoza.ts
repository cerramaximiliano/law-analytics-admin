/**
 * API service for PJ Mendoza worker configuration and statistics.
 * Connects to pjmendoza-api (default https://pjmendoza.lawanalytics.app/api).
 *
...
export type PjMendozaWorkerType = "verifier" | "updater" | "stuck";
...
export interface ILastScaleAction {
	timestamp: string;
	workerType: PjMendozaWorkerType;
...
export const getManager = async (): Promise<IManagerConfig> => {
	const response = await pjmendozaAxios.get("/config/manager");
	return response.data.data;
};

export const getManagerFull = async (): Promise<IManagerFullConfig> => {
	const response = await pjmendozaAxios.get("/config/manager/full");
	return response.data.data;
};

export const updateManagerConfig = async (updates: Partial<IManagerSettings>): Promise<IManagerSettings> => {
	const response = await pjmendozaAxios.patch("/config/manager", updates);
	return response.data.data;
};

export const toggleManager = async (): Promise<{ isRunning: boolean }> => {
	const response = await pjmendozaAxios.post("/config/manager/toggle");
	return response.data.data;
};

export const pauseManager = async (): Promise<{ isPaused: boolean }> => {
	const response = await pjmendozaAxios.post("/config/manager/pause");
	return response.data.data;
};

export const getHistory = async (limit: number = 200): Promise<IHistorySnapshot[]> => {
	const response = await pjmendozaAxios.get("/config/manager/history", { params: { limit } });
	return response.data.data;
};

// Acepta tanto el shape de eje `getAlerts(false)` como el de pjmendoza `getAlerts({ onlyOpen })`.
export const getAlerts = async (
	...
	const response = await pjmendozaAxios.get("/config/manager/alerts", { params });
	return response.data.data;
};

// El UI eje pasa el index (number) del array. pjmendoza-api espera el _id (string).
...
export const acknowledgeAlert = async (
	...
	const response = await pjmendozaAxios.post(`/config/manager/alerts/${alertId}/acknowledge`);
	return response.data.data;
};
...
export const getDailyStats = async (days: number = 30): Promise<IDailyWorkerStats[]> => {
	const response = await pjmendozaAxios.get("/config/manager/daily-stats", { params: { days } });
...
export interface IDailyWorkerStats {
	date: string;
	workerType: PjMendozaWorkerType | "all";
...
function expandDailyStatsForUI(stats: IDailyStats[]): IDailyWorkerStats[] {
	...
	for (const day of stats) {
		const types: PjMendozaWorkerType[] = ["verifier", "updater", "stuck"];
...
export const getAllWorkersConfig = async (): Promise<IAllWorkersResponse> => {
	...
	const [workersResp, managerResp] = await Promise.all([
		pjmendozaAxios.get("/config/manager/workers"),
		pjmendozaAxios.get("/config/manager"),
	]);

	const workersObj = workersResp.data.data as Record<PjMendozaWorkerType, IManagerWorkerConfig>;
	...
	const workers: IWorkerData[] = (Object.keys(workersObj) as PjMendozaWorkerType[]).map((workerType) => {
...
export const updateGlobalSettings = async (updates: Partial<IGlobalSettings>): Promise<IGlobalSettings> => {
	// pjmendoza-api no tiene `/config/manager/settings`. Reusamos PATCH /config/manager.
	const response = await pjmendozaAxios.patch("/config/manager", updates);
	return response.data.data;
};
...
export const getWorkerConfig = async (
	...
	const response = await pjmendozaAxios.get(`/config/manager/worker/${workerType}`);
	return response.data.data;
};
...
export const updateWorkerConfig = async (
	...
	// de pjmendoza-api es minInstances/maxInstances, schedule.enabled (true =
	...
	if (updateThresholdHours !== undefined) {
		await pjmendozaAxios.patch("/config/manager", { updateThresholdHours });
	}
	const response = await pjmendozaAxios.patch(`/config/manager/worker/${workerType}`, body);
	return response.data.data;
};

export const toggleWorker = async (workerType: PjMendozaWorkerType): Promise<{ workerType: string; enabled: boolean }> => {
	const response = await pjmendozaAxios.post(`/config/manager/worker/${workerType}/toggle`);
	return response.data.data;
};

// Worker Stats (raíz, no bajo /config en pjmendoza-api)
export const getWorkerStats = async (): Promise<IWorkerStatsTotals[]> => {
	const response = await pjmendozaAxios.get("/worker-stats");
	return response.data.data;
};
...
export const getTodaySummary = async (): Promise<IDailyWorkerStats[]> => {
	const response = await pjmendozaAxios.get("/worker-stats/today");
	...
	const mapType = (apiType: string): PjMendozaWorkerType => {
...
export const getRunHistory = async (
	...
	const response = await pjmendozaAxios.get(`/worker-stats/${workerType}/runs`, { params });
	return response.data.data;
};

export const getRunById = async (workerType: "verification" | "update" | "stuck", runId: string): Promise<IRunStats> => {
	const response = await pjmendozaAxios.get(`/worker-stats/${workerType}/runs/${runId}`);
	return response.data.data;
};
