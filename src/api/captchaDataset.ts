...
export const CaptchaDatasetService = {
	async list(params: CaptchaDatasetListParams = {}): Promise<CaptchaDatasetListResponse> {
		const response = await pjnAxios.get<CaptchaDatasetListResponse>("/api/captcha-dataset", { params });
		return response.data;
	},

	async stats(): Promise<CaptchaDatasetStatsResponse> {
		const response = await pjnAxios.get<CaptchaDatasetStatsResponse>("/api/captcha-dataset/stats");
		return response.data;
	},

	// URL que el browser usa para cargar el PNG (incluye baseURL).
	// El token de auth viaja por el interceptor de pjnAxios... pero <img src> no
	...
	imageURL(file: string): string {
		...
		const base = (pjnAxios.defaults.baseURL || "").replace(/\/$/, "");
