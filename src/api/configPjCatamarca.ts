/**
 * API service for PJ Catamarca worker configuration and statistics.
 * Connects to pjcatamarca-api (default https://pjcatamarca.lawanalytics.app/api).
 *
...
export type PjCatamarcaWorkerType = "verifier" | "updater" | "stuck";
...
export interface ILastScaleAction {
	timestamp: string;
	workerType: PjCatamarcaWorkerType;
...
export const getManager = async (): Promise<IManagerConfig> => {
	const response = await pjcatamarcaAxios.get("/config/manager");
	return response.data.data;
};

export const getManagerFull = async (): Promise<IManagerFullConfig> => {
	const response = await pjcatamarcaAxios.get("/config/manager/full");
	return response.data.data;
};

export const updateManagerConfig = async (updates: Partial<IManagerSettings>): Promise<IManagerSettings> => {
	const response = await pjcatamarcaAxios.patch("/config/manager", updates);
	return response.data.data;
};

export const toggleManager = async (): Promise<{ isRunning: boolean }> => {
	const response = await pjcatamarcaAxios.post("/config/manager/toggle");
	return response.data.data;
};

export const pauseManager = async (): Promise<{ isPaused: boolean }> => {
	const response = await pjcatamarcaAxios.post("/config/manager/pause");
	return response.data.data;
};

export const getHistory = async (limit: number = 200): Promise<IHistorySnapshot[]> => {
	const response = await pjcatamarcaAxios.get("/config/manager/history", { params: { limit } });
	return response.data.data;
};

// Acepta tanto el shape de eje `getAlerts(false)` como el de pjcatamarca `getAlerts({ onlyOpen })`.
export const getAlerts = async (
	...
	const response = await pjcatamarcaAxios.get("/config/manager/alerts", { params });
	return response.data.data;
};

// El UI eje pasa el index (number) del array. pjcatamarca-api espera el _id (string).
...
export const acknowledgeAlert = async (
	...
	const response = await pjcatamarcaAxios.post(`/config/manager/alerts/${alertId}/acknowledge`);
	return response.data.data;
};
...
export const acknowledgeAllAlerts = async (
	...
	const response = await pjcatamarcaAxios.post("/config/manager/alerts/acknowledge-all", 
		olderThanHours !== undefined ? { olderThanHours } : {});
	return response.data.data;
};

// Retorna stats en el shape eje (`IDailyWorkerStats[]` con una entrada por
// (workerType × día)). Use `getDailyStatsRaw` para el shape nativo pjcatamarca.
export const getDailyStats = async (days: number = 30): Promise<IDailyWorkerStats[]> => {
	const response = await pjcatamarcaAxios.get("/config/manager/daily-stats", { params: { days } });
...
export interface IDailyWorkerStats {
	date: string;
	workerType: PjCatamarcaWorkerType | "all";
...
function expandDailyStatsForUI(stats: IDailyStats[]): IDailyWorkerStats[] {
	...
	for (const day of stats) {
		const types: PjCatamarcaWorkerType[] = ["verifier", "updater", "stuck"];
...
export const getAllWorkersConfig = async (): Promise<IAllWorkersResponse> => {
	...
	const [workersResp, managerResp] = await Promise.all([
		pjcatamarcaAxios.get("/config/manager/workers"),
		pjcatamarcaAxios.get("/config/manager"),
	]);

	const workersObj = workersResp.data.data as Record<PjCatamarcaWorkerType, IManagerWorkerConfig>;
	...
	const workers: IWorkerData[] = (Object.keys(workersObj) as PjCatamarcaWorkerType[]).map((workerType) => {
...
export const updateGlobalSettings = async (updates: Partial<IGlobalSettings>): Promise<IGlobalSettings> => {
	// pjcatamarca-api no tiene `/config/manager/settings`. Reusamos PATCH /config/manager.
	const response = await pjcatamarcaAxios.patch("/config/manager", updates);
	return response.data.data;
};
...
export const getWorkerConfig = async (
	...
	const response = await pjcatamarcaAxios.get(`/config/manager/worker/${workerType}`);
	return response.data.data;
};
...
export const updateWorkerConfig = async (
	...
	// de pjcatamarca-api es minInstances/maxInstances, schedule.enabled (true =
	...
	if (updateThresholdHours !== undefined) {
		await pjcatamarcaAxios.patch("/config/manager", { updateThresholdHours });
	}
	const response = await pjcatamarcaAxios.patch(`/config/manager/worker/${workerType}`, body);
	return response.data.data;
};

export const toggleWorker = async (workerType: PjCatamarcaWorkerType): Promise<{ workerType: string; enabled: boolean }> => {
	const response = await pjcatamarcaAxios.post(`/config/manager/worker/${workerType}/toggle`);
	return response.data.data;
};

// Worker Stats (raíz, no bajo /config en pjcatamarca-api)
export const getWorkerStats = async (): Promise<IWorkerStatsTotals[]> => {
	const response = await pjcatamarcaAxios.get("/worker-stats");
	return response.data.data;
};
...
export const getTodaySummary = async (): Promise<IDailyWorkerStats[]> => {
	const response = await pjcatamarcaAxios.get("/worker-stats/today");
	...
	const mapType = (apiType: string): PjCatamarcaWorkerType => {
...
export const getRunHistory = async (
	...
	const response = await pjcatamarcaAxios.get(`/worker-stats/${workerType}/runs`, { params });
	return response.data.data;
};

export const getRunById = async (workerType: "verification" | "update" | "stuck", runId: string): Promise<IRunStats> => {
	const response = await pjcatamarcaAxios.get(`/worker-stats/${workerType}/runs/${runId}`);
	return response.data.data;
};
