import pjcatamarcaAxios from "utils/pjcatamarcaAxios";

// ========== TYPES ==========

export interface MovimientoPjCatamarca {
	...
	detalle?: string; // no aplica en pjcatamarca
	firmante?: string; // alias singular
}

export interface TraspasoPjCatamarca {
	_id?: string;
	fecha: { $date: string } | string;
	descripcion?: string;
	observacion?: string;
...
export interface IntervinientePjCatamarca {
	vinculo: string;
	nombre: string;
...
export interface CausaRelacionadaPjCatamarca {
...
export type CausaRelacionada = CausaRelacionadaPjCatamarca;
...
export interface CausaPjCatamarca {
	...
	juez?: string;
	juzgado?: string;
	sala?: string; // no aplica en pjcatamarca — alias compat UI eje
	tribunalSuperior?: string; // no aplica en pjcatamarca — alias compat UI eje
	monto?: number; // no aplica en pjcatamarca — alias compat UI eje
	montoMoneda?: string; // no aplica en pjcatamarca — alias compat UI eje
	ubicacionActual?: string;
	expedienteAdministrativoUACF?: string;
	// Sub-docs
	movimiento?: MovimientoPjCatamarca[];
	movimientos?: MovimientoPjCatamarca[]; // alias plural compat con UI eje (no existe en backend)
	movimientosCount?: number;
	traspasos?: TraspasoPjCatamarca[];
	traspasosCount?: number;
	intervinientes?: IntervinientePjCatamarca[];
	causasRelacionadas?: CausaRelacionadaPjCatamarca[];
	// Estado
	estado?: string;
	tieneSentencia?: boolean;
	isPrivate?: boolean;
	// Flags
	source?: "app" | "import" | "pjcatamarca-scraping";
	verified: boolean;
...
export interface CausasPjCatamarcaResponse {
	success: boolean;
	message?: string;
	data: CausaPjCatamarca | CausaPjCatamarca[];
	pagination?: {
		page: number;
		limit: number;
		total: number;
...
function normalizePaginationResponse(resp: CausasPjCatamarcaResponse): CausasPjCatamarcaResponse {
	if (resp?.pagination) {
...
export class CausasPjCatamarcaService {
	...
	static async getPipelineStats(): Promise<PipelineStatsResponse> {
		const response = await pjcatamarcaAxios.get("/causas/pipeline-stats");
		return response.data;
	}

	/** Causas verificadas y válidas (vista Carpetas Verificadas) */
	static async getVerifiedCausas(params?: ListParams): Promise<CausasPjCatamarcaResponse> {
		...
		const response = await pjcatamarcaAxios.get("/causas", { params: queryParams });
		return normalizePaginationResponse(response.data);
	}

	/** Causas no verificadas (verified=false o isValid=false) */
	static async getNonVerifiedCausas(params?: ListParams): Promise<CausasPjCatamarcaResponse> {
	...
	static async getPivotCausas(params?: ListParams): Promise<CausasPjCatamarcaResponse> {
	...
	/** Causa por _id */
	static async getCausaById(id: string): Promise<CausasPjCatamarcaResponse> {
		const response = await pjcatamarcaAxios.get(`/causas/${encodeURIComponent(id)}`);
		return response.data;
	}

	/** Causa por CUIJ */
	static async getCausaByCuij(cuij: string): Promise<CausasPjCatamarcaResponse> {
		const response = await pjcatamarcaAxios.get(`/causas/cuij/${encodeURIComponent(cuij)}`);
		return response.data;
	}
	...
	static async getPivotCandidates(pivotId: string): Promise<{
		success: boolean;
		data: { pivot: Partial<CausaPjCatamarca>; candidates: CausaPjCatamarca[] };
	}> {
		const response = await pjcatamarcaAxios.get(`/causas/${encodeURIComponent(pivotId)}/pivot-candidates`);
		return response.data;
	}
	...
	static async resolvePivot(
		...
		success: boolean;
		data: { pivot: CausaPjCatamarca; selectedCausa: CausaPjCatamarca; foldersMoved: number; foldersSkipped?: number; foldersRemaining?: number };
	}> {
		...
		const response = await pjcatamarcaAxios.post(`/causas/${encodeURIComponent(pivotId)}/resolve-pivot`, { selectedCausaId, ...(folderIds?.length ? { folderIds } : {}) });
		return response.data;
	}
	...
	static async createCausa(payload: {
		...
		userCausaId?: string;
		folderId?: string;
	}): Promise<CausasPjCatamarcaResponse> {
		const response = await pjcatamarcaAxios.post("/causas", payload);
		return response.data;
	}
	...
	static async updateCausa(
		id: string,
		updates: Partial<Pick<CausaPjCatamarca, "update" | "userUpdatesEnabled" | "folderIds" | "userCausaIds">>,
	): Promise<CausasPjCatamarcaResponse> {
		const response = await pjcatamarcaAxios.patch(`/causas/${encodeURIComponent(id)}`, updates);
		return response.data;
	}
	...
	static async forceUpdate(id: string): Promise<{ success: boolean; data: { _id: string; queuedForUpdate: boolean } }> {
		const response = await pjcatamarcaAxios.post(`/causas/${encodeURIComponent(id)}/force-update`);
		return response.data;
	}
	...
	static async resetVerification(id: string): Promise<{ success: boolean; data: { _id: string; queuedForVerification: boolean } }> {
		const response = await pjcatamarcaAxios.post(`/causas/${encodeURIComponent(id)}/reset-verification`);
		return response.data;
	}

	/** Eliminar causa (admin) */
	static async deleteCausa(id: string): Promise<{ success: boolean; data: { _id: string; deleted: boolean } }> {
		const response = await pjcatamarcaAxios.delete(`/causas/${encodeURIComponent(id)}`);
		return response.data;
	}

	/** Alias del UI eje — devuelve las causas candidatas vinculadas al pivote. */
	static async getPivotLinkedCausas(pivotId: string): Promise<CausasPjCatamarcaResponse> {
		const { data } = await CausasPjCatamarcaService.getPivotCandidates(pivotId);
		return { success: true, data: data.candidates };
	}

	/** Stats globales del subsistema pjcatamarca — una sola query agregada en backend. */
	static async getWorkerStats(): Promise<WorkerStatsResponse> {
		try {
			const response = await pjcatamarcaAxios.get("/causas/stats");
			...
			return {
				...
				data: {
					...
					// `details` no aplica en pjcatamarca — devolvemos shape vacío
	...
	static async getEligibilityStats(): Promise<EligibilityStatsResponse> {
		try {
			const response = await pjcatamarcaAxios.get("/causas/eligibility");
			return response.data;
		} catch (error) {
			return {
				success: false,
...
export default CausasPjCatamarcaService;
