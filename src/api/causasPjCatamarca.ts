...
export class CausasPjCatamarcaService {
	...
	static async resolvePivot(
		...
		success: boolean;
		data: { pivot: CausaPjCatamarca; selectedCausa: CausaPjCatamarca; foldersMoved: number };
	}> {
		const response = await pjcatamarcaAxios.post(`/causas/${encodeURIComponent(pivotId)}/resolve-pivot`, { selectedCausaId });
		return response.data;
	}

	/** Crear causa para que el verifier la procese */
	static async createCausa(payload: { searchTerm: string; caratula?: string; userCausaId?: string; folderId?: string }): Promise<CausasPjCatamarcaResponse> {
