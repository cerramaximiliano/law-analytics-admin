import pjmendozaAxios from "utils/pjmendozaAxios";

// ========== TYPES ==========

export interface MovimientoPjMendoza {
	...
	detalle?: string;     // no aplica en pjmendoza
	firmante?: string;    // alias singular
}

export interface TraspasoPjMendoza {
	_id?: string;
	fecha: { $date: string } | string;
	descripcion?: string;
	observacion?: string;
...
export interface IntervinientePjMendoza {
	vinculo: string;
	nombre: string;
...
export interface CausaRelacionadaPjMendoza {
...
export type CausaRelacionada = CausaRelacionadaPjMendoza;
...
export interface CausaPjMendoza {
	...
	juez?: string;
	juzgado?: string;
	sala?: string;            // no aplica en pjmendoza — alias compat UI eje
	tribunalSuperior?: string; // no aplica en pjmendoza — alias compat UI eje
	monto?: number;            // no aplica en pjmendoza — alias compat UI eje
	montoMoneda?: string;      // no aplica en pjmendoza — alias compat UI eje
	ubicacionActual?: string;
	expedienteAdministrativoUACF?: string;
	// Sub-docs
	movimiento?: MovimientoPjMendoza[];
	movimientos?: MovimientoPjMendoza[]; // alias plural compat con UI eje (no existe en backend)
	movimientosCount?: number;
	traspasos?: TraspasoPjMendoza[];
	traspasosCount?: number;
	intervinientes?: IntervinientePjMendoza[];
	causasRelacionadas?: CausaRelacionadaPjMendoza[];
	// Estado
	estado?: string;
	tieneSentencia?: boolean;
	isPrivate?: boolean;
	// Flags
	source?: "app" | "import" | "pjmendoza-scraping";
	verified: boolean;
...
export interface CausasPjMendozaResponse {
	success: boolean;
	message?: string;
	data: CausaPjMendoza | CausaPjMendoza[];
	pagination?: {
		page: number;
		limit: number;
		total: number;
...
function normalizePaginationResponse(resp: CausasPjMendozaResponse): CausasPjMendozaResponse {
	if (resp?.pagination) {
...
export class CausasPjMendozaService {
	/** Causas verificadas y válidas (vista Carpetas Verificadas) */
	static async getVerifiedCausas(params?: ListParams): Promise<CausasPjMendozaResponse> {
		...
		const response = await pjmendozaAxios.get("/causas", { params: queryParams });
		return normalizePaginationResponse(response.data);
	}

	/** Causas no verificadas (verified=false o isValid=false) */
	static async getNonVerifiedCausas(params?: ListParams): Promise<CausasPjMendozaResponse> {
	...
	static async getPivotCausas(params?: ListParams): Promise<CausasPjMendozaResponse> {
	...
	/** Causa por _id */
	static async getCausaById(id: string): Promise<CausasPjMendozaResponse> {
		const response = await pjmendozaAxios.get(`/causas/${encodeURIComponent(id)}`);
		return response.data;
	}

	/** Causa por CUIJ */
	static async getCausaByCuij(cuij: string): Promise<CausasPjMendozaResponse> {
		const response = await pjmendozaAxios.get(`/causas/cuij/${encodeURIComponent(cuij)}`);
		return response.data;
	}
	...
	static async getPivotCandidates(pivotId: string): Promise<{
		success: boolean;
		data: { pivot: Partial<CausaPjMendoza>; candidates: CausaPjMendoza[] };
	}> {
		const response = await pjmendozaAxios.get(`/causas/${encodeURIComponent(pivotId)}/pivot-candidates`);
		return response.data;
	}
	...
	static async createCausa(payload: { searchTerm: string; caratula?: string; userCausaId?: string; folderId?: string }): Promise<CausasPjMendozaResponse> {
		const response = await pjmendozaAxios.post("/causas", payload);
		return response.data;
	}
	...
	static async updateCausa(
		id: string,
		updates: Partial<Pick<CausaPjMendoza, "update" | "userUpdatesEnabled" | "folderIds" | "userCausaIds">>,
	): Promise<CausasPjMendozaResponse> {
		const response = await pjmendozaAxios.patch(`/causas/${encodeURIComponent(id)}`, updates);
		return response.data;
	}
	...
	static async forceUpdate(id: string): Promise<{ success: boolean; data: { _id: string; queuedForUpdate: boolean } }> {
		const response = await pjmendozaAxios.post(`/causas/${encodeURIComponent(id)}/force-update`);
		return response.data;
	}
	...
	static async resetVerification(id: string): Promise<{ success: boolean; data: { _id: string; queuedForVerification: boolean } }> {
		const response = await pjmendozaAxios.post(`/causas/${encodeURIComponent(id)}/reset-verification`);
		return response.data;
	}

	/** Eliminar causa (admin) */
	static async deleteCausa(id: string): Promise<{ success: boolean; data: { _id: string; deleted: boolean } }> {
		const response = await pjmendozaAxios.delete(`/causas/${encodeURIComponent(id)}`);
		return response.data;
	}

	/** Alias del UI eje — devuelve las causas candidatas vinculadas al pivote. */
	static async getPivotLinkedCausas(pivotId: string): Promise<CausasPjMendozaResponse> {
		const { data } = await CausasPjMendozaService.getPivotCandidates(pivotId);
		return { success: true, data: data.candidates };
	}

	/** Stats globales del subsistema pjmendoza — una sola query agregada en backend. */
	static async getWorkerStats(): Promise<WorkerStatsResponse> {
		try {
			const response = await pjmendozaAxios.get("/causas/stats");
			...
			return {
				...
				data: {
					...
					// `details` no aplica en pjmendoza — devolvemos shape vacío
	...
	static async getEligibilityStats(): Promise<EligibilityStatsResponse> {
		try {
			const response = await pjmendozaAxios.get("/causas/eligibility");
			return response.data;
		} catch (error) {
			return {
				success: false,
...
export default CausasPjMendozaService;
