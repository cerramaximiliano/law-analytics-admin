...
class WorkerConfigService {
	...
	async getConfigs(params?: {
		...
		try {
			const response = await pjnAxios.get(this.endpoint, { params });
			return response.data;
		} catch (error) {
			throw WorkersPjnService.handleError(error);
		}
	}

	async updateConfig(id: string, data: Partial<WorkerConfig>): Promise<WorkerConfigResponse> {
		try {
			const response = await pjnAxios.put(`${this.endpoint}${id}`, data);
			return response.data;
		} catch (error) {
			throw WorkersPjnService.handleError(error);
		}
	...
	async updateRange(id: string, data: { range_start: number; range_end: number }): Promise<WorkerConfigResponse> {
		try {
			const response = await pjnAxios.put(`${this.endpoint}${id}/range`, data);
			return response.data;
		} catch (error) {
			throw WorkersPjnService.handleError(error);
		}
	...
	async deleteConfig(id: string): Promise<WorkerConfigResponse> {
		try {
			const response = await pjnAxios.delete(`${this.endpoint}${id}`);
			return response.data;
		} catch (error) {
			throw WorkersPjnService.handleError(error);
		}
...
export class WorkersPjnService {
	...
	static async getScrapingHistory(params?: {
		...
		try {
			const response = await pjnAxios.get("/api/configuracion-scraping-history/", { params });
			return response.data;
		} catch (error) {
	...
	static handleError(error: any): Error {
		...
		// Esto permite que el interceptor de pjnAxios intente refrescar el token en errores 401
...
export default WorkersPjnService;
