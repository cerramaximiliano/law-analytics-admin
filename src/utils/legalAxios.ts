...
// Instancia de Axios para la API Legal
const legalAxios: AxiosInstance = axios.create({
	baseURL: import.meta.env.VITE_API_LEGAl || "https://legal.lawanalytics.app",
	timeout: 30000,
	headers: {
...
legalAxios.interceptors.request.use(
	...
	async (error) => {
		...
		if (error.response?.status === 401 && !originalRequest._retry && !originalRequest._queued) {
			...
			try {
				...
				return legalAxios(originalRequest);
			} catch (refreshError) {
